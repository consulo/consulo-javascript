...
public abstract class BaseCreateFix implements LocalQuickFix {
    ...
    public static void guessExprTypeAndAddSuchVariable(
        final JSExpression passedParameterValue,
        final Template template,
        final String var1,
        final PsiFile file,
    ...
    protected void guessTypeAndAddTemplateVariable(Template template, JSExpression referenceExpression, PsiFile file) {
        ...
        if (parent instanceof JSDefinitionExpression) {
            ...
            if (grandParent instanceof JSAssignmentExpression assignment) {
        ...
        String expressionType = elementForWhichExprTypeToEvaluate instanceof JSExpression expression
            ? JSResolveUtil.getExpressionType(expression, file) : null;
    ...
    protected static JSClass findClass(PsiFile file, final PsiElement anchorParent) {
        return anchorParent instanceof JSClass jsClass ? jsClass
            : file instanceof JSFile jsFile ? JSResolveUtil.getXmlBackedClass(jsFile)
            : null;
    }
