...
public class JSResolveUtil {
    ...
    private static UserDataCache<CachedValue<IntObjectMap<Object>>, JSElement, Object> ourCachedDefsCache =
        ...
        if (element != null) {
            if (element instanceof JSFunction) {
                final PsiElement elt =
                    JSDocumentationUtils.findDocComment(JSDocumentationProvider.findElementForWhichPreviousCommentWillBeSearched(element));
    ...
    public static boolean shouldProcessTopLevelGlobalContext(@Nonnull PsiElement place, @Nonnull PsiScopeProcessor processor) {
        ...
        return shouldProcessImports(place, processor) && (
            !((placeParent = place.getParent()) instanceof JSCallExpression)
                || (place instanceof JSReferenceExpression referenceExpression && referenceExpression.getQualifier() != null
    ...
    public static Set<String> calculateOpenNses(PsiElement place) {
        final Ref<Set<String>> result = new Ref<>();
        ...
        if (elt instanceof PsiFile) {
            if (elt instanceof XmlFile) {
                ...
                if (JavaScriptSupportLoader.isMxmlNs(rootNs)) {
                    ...
                    if (rootTag != null && xmlFile.getFileType() == XmlFileType.INSTANCE) { // TODO this is bindows specific
                        processXmlFile(processor, xmlFile, place);
                    }
                }
            }
    ...
    private static UserDataCache<CachedValue<List<JSVariable>>, XmlFile, Object> ourCachedPredefinedVars =
        new UserDataCache<CachedValue<List<JSVariable>>, XmlFile, Object>() {
            @Override
    ...
    public static JSClass findDeclaringClass(final JSFunction method) {
        final Ref<JSClass> lastVisitedClass = new Ref<>();
        iterateOverridenMethodsUp(
            method,
            new Processor<JSClass>() {
                @Override
            ...
            for (ResolveResult r : resolveResults) {
                ...
                if (resolvedElement.isEquivalentTo(element) ||
                    ...
                    if (reference instanceof JSReferenceExpression referenceExpression
                        ...
                        || resolvedElement instanceof JSFunctionExpression functionExpression
                        && functionExpression.getFunction().getNameIdentifier().getNextSibling() == reference)) {
                ...
                if (resolvedElement instanceof JSFunctionExpression functionExpression) {
                    final PsiElement nameIdentifier = functionExpression.getFunction().getNameIdentifier();
                    ...
                    if (fun.isConstructor()) {
                        if ((element instanceof JSClass && resolvedElement.getParent() == element)
                            || (element instanceof JSFile && reference.getElement().getParent() != resolvedElement
                        ...
                        if (elementParent instanceof JSClass && resolvedElementParent instanceof JSClass) {
                            ...
                            final Collection<JSClass> visitedInterfaces = new HashSet<>();
                ...
                if (
                    (resolvedElement instanceof JSClass
                        ...
                        && ((element instanceof XmlFile && resolvedElement.getParent().getContainingFile() == element)
                        || (element instanceof JSFile && reference.getElement().getParent() != resolvedElement
    ...
    private static JSNamedElement findOverriddenMethod(PsiElement resolvedElement, JSClass resolvedElementClass) {
        final Ref<JSNamedElement> override = new Ref<>();
    ...
    public static JSExpression getRealRefExprQualifierFromResult(
        ...
        final JSImportedElementResolveResult resolved
    ) {
    ...
    public static String findPackageStatementQualifier(final PsiElement context) {
        if (context instanceof JSClass jsClass) {
            final String s = jsClass.getQualifiedName();
            if (s != null) {
        ...
        synchronized (project) {
            ...
            for (JSQualifiedNamedElement clazz : candidates) {
                if (link.equals(clazz.getQualifiedName())) {
                    if ("Object".equals(link) && !JavaScriptIndex.ECMASCRIPT_JS2.equals(clazz.getContainingFile()
                        .getVirtualFile()
                        .getName()) // object from swf do
    ...
    private static boolean isBuiltInClassName(final String className) {
        return OBJECT_CLASS_NAME.equals(className) || "Boolean".equals(className) || "Function".equals(className)
    ...
    public static boolean iterateType(
        final JSFunction node,
        final PsiElement jsClass,
        final String typeName,
        final OverrideHandler handler
    ) {
    ...
    public static boolean isInPlaceWhereTypeCanBeDuringCompletion(PsiElement expr) {
        ...
        if (parent instanceof JSExpressionStatement) {
            return expr instanceof JSReferenceExpression referenceExpression ? referenceExpression.getQualifier() == null : true;
        }
        return false;
    }
    ...
    public static boolean isPlaceWhereNsCanBe(final PsiElement parent) {
        ...
        return grandParent instanceof JSClass || grandParent instanceof JSPackageStatement
            || (grandParent instanceof JSFile && grandParent.getContext() == null);
    }

    @Nullable
    public static String getTypeFromTagNameInMxml(final @Nullable PsiElement psiElement) {
    ...
    private static boolean doProcessMetaAttributesForClass(
        ...
        if (jsClass instanceof JSClass jsClassClass) {
            ...
            final JSAttributeList attributeList = jsClassClass.getAttributeList();
        ...
        Ref<PsiElement> continuePassElement = new Ref<>();
    ...
    public static boolean processAttributeList(
        final MetaDataProcessor processor,
        final PsiElement el,
        final JSAttributeList attributeList,
        boolean forward
    ) {
    ...
    public static JSDefinitionExpression getDefinitionExpr(JSExpressionStatement exprStatement) {
        ...
        if (expression instanceof JSAssignmentExpression assignment) {
            return (JSDefinitionExpression)assignment.getLOperand();
        }
        return null;
    }

    public static boolean processDeclarationsInScope(
        final JSElement _scope,
        final PsiScopeProcessor processor,
    ...
    public static class RelevantDefsUserDataCache extends UserDataCache<CachedValue<IntObjectMap<Object>>, JSElement, Object> {
        ...
        protected CachedValue<IntObjectMap<Object>> compute(final JSElement jsElement, final Object o) {
            return CachedValuesManager.getManager(jsElement.getProject()).createCachedValue(
                new CachedValueProvider<IntObjectMap<Object>>() {
                    @Override
        ...
        private static class MyJSElementVisitor extends JSElementVisitor {
            ...
            public void visitJSDefinitionExpression(final JSDefinitionExpression node) {
                ...
                if (definedExpr instanceof JSReferenceExpression referenceExpression && referenceExpression.getQualifier() == null && false) {
            ...
            private void addNamedElement(final JSElement node, final int key) {
                ...
                if (o == null) {
                    ...
                    final JSElement[] newO = new JSElement[]{element, node};
        ...
        private void resolveContext(final PsiElement context) {
            ...
            if (parentContainer instanceof JSFunctionExpression) {
                ...
                while (parentContainer instanceof JSFunctionExpression) {
                    ...
                    if (parentContainerParent instanceof JSAssignmentExpression assignment) {
                        final JSExpression jsExpression = assignment.getLOperand();
                        final JSElement functionExpressionName =
                        ...
                        if (functionExpressionName instanceof JSReferenceExpression referenceExpression) {
                            final JSExpression functionExpressionNameQualifier = referenceExpression.getQualifier();
                            ...
                            else if (functionExpressionNameQualifier instanceof JSReferenceExpression functionReferenceExpression) {
                                final String functionExpressionNameQualifierText = functionReferenceExpression.getReferencedName();

                                if (PROTOTYPE_FIELD_NAME.equals(functionExpressionNameQualifierText)) {
                                    qualifyingExpression = functionReferenceExpression.getQualifier();
                                }
                                else if (!changedFunctionScope) {
                                    String referencedName;

                                    if (functionReferenceExpression.getQualifier() == null
                                        && ((referencedName = referenceExpression.getReferencedName()) == null
                        ...
                        final JSElement element = PsiTreeUtil.getParentOfType(
                            parentContainerParent,
                            JSVariable.class,
            ...
            if (parentContainer instanceof JSFunctionExpression && parentContainer.getContainingFile()
                .getLanguage() == JavaScriptSupportLoader.ECMA_SCRIPT_L4) {
    ...
    public static void processGlobalSymbols(PsiElement target, PsiScopeProcessor processor) {
        ...
        if (sdk != null) {
            ...
            for (VirtualFile file : files) {
                ...
                if (psiFile instanceof JSFile && !psiFile.processDeclarations(processor, ResolveState.initial(), null, target)) {
                    return;
                }
            }
        }
