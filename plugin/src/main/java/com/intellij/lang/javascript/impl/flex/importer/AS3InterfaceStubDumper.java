...
class AS3InterfaceStubDumper extends AS3InterfaceDumper {
    ...
    public void processParameter(
        ...
        /*new JSParameterStubImpl(
            ...
            getMultinameAsPackageName(type, parentName, true),
            getValueRepr(value),
            parents.getLast()
        );*/
    }
    ...
    public void processFunction(MethodInfo methodInfo, boolean referenceNameRequested, Abc abc, String indent, String attr) {
        /*parents.add(new JSFunctionStubImpl(
            methodInfo.name.name,
            methodInfo.isGetMethod()
                ? JSFunctionStubImpl.GET_PROPERTY_MASK
                : methodInfo.isSetMethod()
                ? JSFunctionStubImpl.SET_PROPERTY_MASK
                : methodInfo.parentTraits != null && methodInfo.parentTraits.name == methodInfo.name
                ? JSFunctionStubImpl.CONSTRUCTOR_MASK
                : 0,
            getMultinameAsPackageName(
                methodInfo.name,
                methodInfo.parentTraits != null ? methodInfo.parentTraits.getClassName() : null,
                referenceNameRequested
            ),
            getMultinameAsPackageName(
                methodInfo.returnType,
                methodInfo.getParentName(),
                referenceNameRequested
            ),
            parents.getLast()
        ));
    ...
    public void processVariable(SlotInfo info, String indent, String attr) {
        ...
        new JSVariableStubImpl(
            qName.substring(qName.lastIndexOf('.') + 1), info.isConst() ? JSVariableStubImpl.CONST_MASK : 0,
    ...
    public void processClass(SlotInfo slotInfo, Abc abc, String attr, String indent) {
        /*parents.add(
            new JSClassStubImpl(slotInfo.name.name, slotInfo.isInterfaceClass() ? JSClassStubImpl.INTERFACE_MASK : 0,
    ...
    protected void processModifierList(MemberInfo memberInfo, String attr, String indent) {
        ...
        parents.add(new JSAttributeListStubImpl(
            parents.getLast(),
            ns,
            accessType,
            modifiers.toArray(new JSAttributeList.ModifierType[modifiers.size()])
        ));
