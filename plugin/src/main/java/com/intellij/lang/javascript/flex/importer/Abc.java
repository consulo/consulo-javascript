...
class Abc
{
		...
		for(i = 1; i < n; i++)
		{
			switch(data.readByte())
			{
				...
				case CONSTANT_TypeName:
					// TODO:
					int nameId = readU32();
					String nsName = names[nameId].toString();
					...
					names[i] = new Multiname(new String[]{index != -1 ? nsName.substring(0, index) : ""}, index != -1 ? nsName.substring(index + 2) : nsName);
					break;
	...
	void parseMethodInfos()
	{
		...
		for(int i = 0; i < method_count; i++)
		{
			...
			if((m.flags & NEED_REST) != 0)
			{
				...
				{
					m.paramNames = ArrayUtil.append(m.paramNames, "__rest");
				}
			}
		}
	...
	void parseInstanceInfos()
	{
		...
		for(int i = 0; i < count; i++)
		{
			...
			if((t.flags & CLASS_FLAG_protected) != 0)
			{
	...
	void parseTraits(Traits t)
	{
		...
		for(int i = 0; i < namecount; i++)
		{
			...
			if(member == null)
			{
				processor.hasError("error trait kind " + kind + "\n");
			}
			member.kind = kind;
			member.name = name;
	...
	void parseMethodBodies()
	{
		...
		for(int i = 0; i < count; i++)
		{
			...
			if(code_length > 0)
			{
				data.readBytes(m.code, code_length);
			}
