...
import static com.intellij.lang.javascript.impl.flex.importer.Abc.*;
	...
	protected void dumpCode(Abc abc, String indent, FlexByteCodeInformationProcessor processor)
	{
		...
		while(!code.eof())
		{
			...
			switch(opcode)
			{
				case OP_debugfile:
				case OP_pushstring:
					processor.append('"' + abc.strings[readU32()].replaceAll("/\n/g", "\\n").replaceAll("/\t/g", "\\t") + '"');
					break;
				...
				case OP_ifstrictne:
					...
					if(!(labels.containsKey(code.getPosition())))
					{
						processor.append("\n");
					}
					break;
