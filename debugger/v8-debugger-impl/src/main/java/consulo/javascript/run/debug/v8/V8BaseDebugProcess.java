...
public abstract class V8BaseDebugProcess<V extends JavascriptVm> extends XDebugProcess {
    ...
    public V8BaseDebugProcess(XDebugSession session, ExecutionResult result) throws ExecutionException {
        super(session);
        myScriptListPanel = new JavaScriptListPanel<Script>(session.getProject()) {
            @Nullable
            @Override
            public VirtualFile toVirtualFile(Script value, boolean toOpen) {
                ...
                public void registerBreakpoint(final XLineBreakpoint lineBreakpoint) {
                    ...
                    myVm.setBreakpoint(new Breakpoint.Target.ScriptName(presentableFilePath), line, Breakpoint.EMPTY_VALUE, true, expression, new JavascriptVm.BreakpointCallback() {
                        ...
                        public void failure(String s) {
                            myXBreakpointManager.updateBreakpointPresentation(lineBreakpoint, ExecutionDebugIconGroup.breakpointBreakpointinvalid(), s);
                        }
                    }, null);
                }
