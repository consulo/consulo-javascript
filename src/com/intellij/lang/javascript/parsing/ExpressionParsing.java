...
public class ExpressionParsing extends Parsing{
  ...
  private static boolean parsePrimaryExpression(PsiBuilder builder) {
    ...
    if (firstToken == JSTokenTypes.THIS_KEYWORD) {
    ...
    else if (isIdentifierToken(firstToken) ||
             firstToken == JSTokenTypes.ANY_IDENTIFIER
            ) {
      ...
      if (JSTokenTypes.COLON_COLON == builder.getTokenType()) {
        builder.advanceLexer();
        if (isIdentifierToken(builder.getTokenType())) builder.advanceLexer();
        marker.done(JSElementTypes.REFERENCE_EXPRESSION);
      } else {
        marker.drop();
      }
      return true;
    } else if (firstToken == JSTokenTypes.XML_START_TAG_START ||
               firstToken == JSTokenTypes.XML_START_TAG_LIST) {
      parseTag(builder);
      return true;
  ...
  private static void parseTag(final PsiBuilder builder) {
    ...
    try {
      ...
      for(IElementType currentTokenType = builder.getTokenType();
  ...
  private static boolean isNotPropertyStart(final IElementType elementType) {
    return !JSTokenTypes.IDENTIFIER_TOKENS_SET.contains(elementType) && elementType != JSTokenTypes.STRING_LITERAL && elementType != JSTokenTypes.NUMERIC_LITERAL;
  }
    ...
    try {
      ...
      if (type == JSTokenTypes.NEW_KEYWORD) {
        ...
        if (isfunction) {
          ...
          if (builder.getTokenType() != JSTokenTypes.LPAR) return true;
          expr = expr.precede();
          isNew = false;
        }
      }
      ...
      while (true) {
        IElementType tokenType = builder.getTokenType();
        if (tokenType == JSTokenTypes.DOT ||
            (tokenType == JSTokenTypes.COLON_COLON || tokenType == JSTokenTypes.DOT_DOT)
           ) {
          ...
          if (isNew) expr.done(JSElementTypes.NEW_EXPRESSION);
          else expr.drop();
          break;
        }
      }
      ...
      if (gwtExprMark != null) gwtExprMark.done(JSElementTypes.GWT_REFERENCE_EXPRESSION);
    }
  ...
  public static boolean parseQualifiedTypeName(PsiBuilder builder, boolean allowStar) {
    if (!JSTokenTypes.IDENTIFIER_TOKENS_SET.contains(builder.getTokenType())) return false;
    PsiBuilder.Marker expr = builder.mark();
    buildTokenElement(JSElementTypes.REFERENCE_EXPRESSION, builder);
  ...
  private static boolean parseAssignmentExpression(final PsiBuilder builder, boolean allowIn) {
    ...
    if (JSTokenTypes.ASSIGNMENT_OPERATIONS.contains(builder.getTokenType()) &&
        builder.getUserData(WITHIN_OBJECT_LITERAL_EXPRESSION) == null) {
      builder.error(JSBundle.message("javascript.parser.message.expected.expression"));
      builder.advanceLexer();
  ...
  private static boolean parseIsAsExpression(final PsiBuilder builder) {
    ...
    while (builder.getTokenType() == JSTokenTypes.AS_KEYWORD ||
           builder.getTokenType() == JSTokenTypes.IS_KEYWORD
          ) {
      builder.advanceLexer();
      if (!parseUnaryExpression(builder)) {
        builder.error(JSBundle.message("javascript.parser.message.expected.expression"));
