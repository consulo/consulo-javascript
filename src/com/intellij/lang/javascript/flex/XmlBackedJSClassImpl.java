...
public class XmlBackedJSClassImpl extends JSClassBase implements JSClass {
  ...
  @NonNls public static final String COMPONENT_TAG_NAME = "Component";
  @NonNls public static final String CLASS_NAME_ATTRIBUTE_NAME = "className";
  ...
  protected boolean processMembers(final PsiScopeProcessor processor, final ResolveState substitutor, final PsiElement lastParent, final PsiElement place) {
    for(JSFile file: ourCachedScripts.get(CACHED_SCRIPTS_KEY, getParent(), null).getValue()) {
      if (!file.processDeclarations(processor, ResolveState.initial(), null, place)) return false;
    }
    return true;
  }
  ...
  public boolean doImportFromScripts(final PsiScopeProcessor processor, final PsiElement place) {
    ...
    if ((context instanceof XmlAttributeValue && (!(place instanceof JSReferenceExpression) || !JSResolveUtil.referenceExpressionShouldBeQualified((JSReferenceExpression)place))) ||
      ...
      for(JSFile file: files) {
        if(JSImportHandlingUtil.isAdequatePlaceForImport(file, place)) {
          if (useImports && JSImportHandlingUtil.importClass(processor, file)) return false;
          adequatePlace = true;
        }
      }

      if (adequatePlace && processor instanceof ResolveProcessor) {
        if (!processComponentNames((ResolveProcessor)processor)) return false;
      }
    }
    ...
    if (processor instanceof ResolveProcessor && JSResolveUtil.shouldProcessTopLevelGlobalContext(place, processor) &&
      ...
      if(!JSResolveUtil.processGlobalThings(processor, ResolveState.initial(), place, this)) return false;
    }
    return true;
  }
  ...
  public boolean processComponentNames(ResolveProcessor processor) {
    ...
    if (s != null) {
      ...
      if (qName != null) {
        ...
        if (clazz != null) return processor.execute(clazz, ResolveState.initial());
      }
    } else {
      for(String qName:value.values()) {
        PsiElement clazz = JSResolveUtil.findClassByQName(qName, this);
        if (clazz != null && !processor.execute(clazz, ResolveState.initial())) return false;
      }
    }
    return true;
  }
  ...
  private static final UserDataCache<CachedValue<JSFile[]>,XmlTag, Object> ourCachedScripts =
      new UserDataCache<CachedValue<JSFile[]>, XmlTag, Object>() {
  ...
  private static final UserDataCache<CachedValue<Map<String, String>>,XmlFile, Object> myCachedComponentImportsCache =
      new UserDataCache<CachedValue<Map<String, String>>, XmlFile, Object>() {
  ...
  private static final UserDataCache<ParameterizedCachedValue<XmlBackedJSClassImpl, XmlTag>,XmlTag, Object> myCachedClassCache =
    new UserDataCache<ParameterizedCachedValue<XmlBackedJSClassImpl, XmlTag>, XmlTag, Object>() {
      protected ParameterizedCachedValue<XmlBackedJSClassImpl, XmlTag> compute(final XmlTag tag, final Object p) {
        return CachedValuesManager.getManager(tag.getProject()).createParameterizedCachedValue(
          new ParameterizedCachedValueProvider<XmlBackedJSClassImpl, XmlTag>() {
  ...
  public boolean isEquivalentTo(final PsiElement element2) {
    return this == element2 || 
           ...
           (element2 instanceof XmlBackedJSClassImpl &&
            getContainingFile() == element2.getContainingFile());
  }
        ...
        if (child instanceof PsiWhiteSpace &&
          ...
          if ((cdataAt = text.indexOf(marker)) == -1) {
            ...
            ASTNode fromText = JSChangeUtil.createJSTreeFromText(
              ...
              (JSLanguageDialect) element.getContainingFile().getLanguage()
            );
  ...
  public PsiElement addBefore(@NotNull final PsiElement element, final PsiElement anchor) throws IncorrectOperationException {
    if (anchor == null) return add(element);
