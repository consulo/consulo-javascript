...
class AS3InterfaceStubDumper extends AS3InterfaceDumper {
  ...
  public void processFunction(MethodInfo methodInfo, boolean referenceNameRequested, Abc abc, String indent, String attr) {
    parents.add(
      new JSFunctionStubImpl(
        ...
        getMultinameAsPackageName(methodInfo.name,methodInfo.parentTraits != null ? methodInfo.parentTraits.getClassName():null, referenceNameRequested), 
        getMultinameAsPackageName(methodInfo.returnType, methodInfo.getParentName(),referenceNameRequested),
        parents.getLast()
      )
    );
  ...
  public void processVariable(SlotInfo info, String indent, String attr) {
    ...
    new JSVariableStubImpl(qName.substring(qName.lastIndexOf('.') + 1),
      ...
      getMultinameAsPackageName(info.type, parentName, false),
  ...
  public void processClass(SlotInfo slotInfo, Abc abc, String attr, String indent) {
    parents.add(
      new JSClassStubImpl(
        ...
        getMultinameAsPackageName(slotInfo.name, null, false),
        parents.getLast()
      )
    );
