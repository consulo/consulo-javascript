...
class AS3InterfaceDumper extends AbstractDumpProcessor {
  ...
  public boolean doDumpMember(final @NotNull MemberInfo memberInfo) {
    ...
    if (memberInfo.name.name != null && memberInfo.name.name.indexOf(Abc.$CINIT) >= 0) return false;
    return true;
  }
  ...
  private static @NonNls Set<String> doNotNeedQoting = new THashSet<String>(Arrays.asList("null", "NaN", "undefined", "true", "false", "Infinity", "-Infinity"));
    ...
    if (name.hasNotEmptyNs()) {
      if (name.hasNamespace() ||
          (!verbose && parentName != null &&
             (parentName.equals(name.nsset[0].replaceAll(":","::"))) ||
             ( parentName != null && parentName.equals(name.toString())))
        ) {
        return name.name;
      }
  ...
  protected String appendModifiers(MemberInfo member, String attr) {
    ...
    if (s.indexOf("private") == -1 && !hasNs && !isInterface) {
      ...
      if(member.isPublic ||
         ...
         (member.parentTraits != null &&
            ( (parentName = member.parentTraits.name.toString()).equals(member.name.toString()) ||
              parentName.startsWith("script")
            )
         )
         ) {
        s+="public ";
