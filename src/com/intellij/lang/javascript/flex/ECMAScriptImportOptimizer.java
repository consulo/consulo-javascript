...
public class ECMAScriptImportOptimizer implements ImportOptimizer
{
  ...
  public Runnable processFile(final PsiFile file) {
    if (!CodeInsightUtilBase.getInstance().prepareFileForWrite(file)) return EmptyRunnable.INSTANCE;
        ...
        try {
          ...
          for (Pair<SmartPsiElementPointer<JSElement>, Collection<String>> entry : importsByHolderPointer) {
            ...
            PsiElement newImports = PsiFileFactory.getInstance(project)
              .createFileFromText("dummy." + JavaScriptSupportLoader.ECMA_SCRIPT_L4.getFileExtension(), importBlock);
            ...
            PsiFile realFile = file.getContext() != null ? file.getContext().getContainingFile() : file;
			  ...
			  CodeStyleManager.getInstance(project).reformatText(realFile, textRange.getStartOffset(),
          ...
          for (SmartPsiElementPointer pointer : oldImportsPointers) {
            ...
            if (statement != null) deleteImport(statement);
          }
