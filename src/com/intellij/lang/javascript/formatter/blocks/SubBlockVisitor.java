...
public class SubBlockVisitor extends JSNodeVisitor {
  ...
  @Override public void visitElement(final ASTNode node) {
  ...
  @Override public void visitDocComment(final ASTNode node) {
    final ASTNode child = node.getFirstChildNode();
    if (child != null && child.getElementType() == JSTokenTypes.DOC_COMMENT) visit(child);
    else super.visitDocComment(node);
  }

  @Override public void visitComment(final ASTNode node) {
  ...
  private Indent getIndent(final ASTNode node, final ASTNode child) {
    ...
    if (nodeElementType instanceof JSFileElementType ||
    ...
    if (nodeElementType == JSElementTypes.PACKAGE_STATEMENT && !JSSpacingProcessor.NOT_A_PACKAGE_CONTENT.contains(childElementType)) {
      final JSCodeStyleSettings customSettings = CodeStyleSettingsManager.getSettings(node.getPsi().getProject()).getCustomSettings(
