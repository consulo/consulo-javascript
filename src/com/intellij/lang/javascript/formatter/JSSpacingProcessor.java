...
public class JSSpacingProcessor extends JSNodeVisitor {
  ...
  private boolean shouldFormatObjectLiteralExpression() {
    ...
    if (grandParentType == JSElementTypes.ARRAY_LITERAL_EXPRESSION) return true;
  ...
  private boolean shouldFormatArrayLiteralExpression() {
    ...
    if (node != null) {
      ...
      return type == JSElementTypes.ARRAY_LITERAL_EXPRESSION ||
  ...
  @Override public void visitEmbeddedContent(final ASTNode node) {
  ...
  @Override public void visitParameterList(final ASTNode node) {
  ...
  @Override public void visitBlock(final ASTNode node) {
    processBlock(node);
  }
    ...
    if (JSElementTypes.SOURCE_ELEMENTS.contains(type1) || JSElementTypes.SOURCE_ELEMENTS.contains(type2) ||
      ...
      if ( isInjectedJSHack(type1, type2) ||
        ...
        final boolean keepOneLine = myParent.getPsi() instanceof JSFunction
                                    ? mySettings.KEEP_SIMPLE_METHODS_IN_ONE_LINE:mySettings.KEEP_SIMPLE_BLOCKS_IN_ONE_LINE;
        ...
        if (keepOneLine && (type1 == JSTokenTypes.LBRACE || type2 == JSTokenTypes.RBRACE)) {
          ...
          if ((JSElementTypes.IMPORT_STATEMENT == type1 && JSElementTypes.IMPORT_STATEMENT != type2 && JSTokenTypes.RBRACE != type2) ||
            (JSElementTypes.IMPORT_STATEMENT != type1 && JSTokenTypes.LBRACE != type1 && JSElementTypes.IMPORT_STATEMENT == type2)) {
            ...
            if (shouldPlaceExtraLinesAroundMethod()) {
              myResult = Spacing.createSpacing(0, 0, 1 + mySettings.BLANK_LINES_AROUND_METHOD, mySettings.KEEP_LINE_BREAKS, mySettings.KEEP_BLANK_LINES_IN_CODE);
            } else {
              myResult = Spacing.createSpacing(0, 0, 1, mySettings.KEEP_LINE_BREAKS, mySettings.KEEP_BLANK_LINES_IN_CODE);
            }
          }
  ...
  @Override public void visitFile(final ASTNode node) {
    if (JSElementTypes.SOURCE_ELEMENTS.contains(type1) || JSElementTypes.SOURCE_ELEMENTS.contains(type2)) {
      if (type2 == JSTokenTypes.END_OF_LINE_COMMENT && isInlineEndOfLineCommentOnLeft() && type1 != JSElementTypes.IMPORT_STATEMENT) {
        myResult = Spacing.getReadOnlySpacing();
      } else {
        if ((JSElementTypes.IMPORT_STATEMENT == type1 && JSElementTypes.IMPORT_STATEMENT != type2) ||
          (JSElementTypes.IMPORT_STATEMENT != type1 && JSElementTypes.IMPORT_STATEMENT == type2)) {
          myResult = Spacing.createSpacing(0, 0, 2, mySettings.KEEP_LINE_BREAKS, mySettings.KEEP_BLANK_LINES_IN_CODE);
        }
          else if (JSElementTypes.IMPORT_STATEMENT == type1 && JSElementTypes.IMPORT_STATEMENT == type2) {
            myResult = getSpacingBetweenImports();
  ...
  @Override public void visitFunctionDeclaration(final ASTNode node) {
  ...
  @Override public void visitFunctionExpression(final ASTNode node) {
    visitFunctionDeclaration(node);
  }

  @Override public void visitReferenceExpression(final ASTNode node) {
  ...
  @Override public void visitIfStatement(final ASTNode node) {
  ...
  @Override public void visitCallExpression(final ASTNode node) {
    if (type2 == JSElementTypes.ARGUMENT_LIST) {
      setSingleSpace(mySettings.SPACE_BEFORE_METHOD_CALL_PARENTHESES);
    }
  }

  @Override public void visitNewExpression(final ASTNode node) {
    if (type1 == JSTokenTypes.NEW_KEYWORD) {
      setSingleSpace(true);
    }
  ...
  @Override public void visitForStatement(final ASTNode node) {
  ...
  @Override public void visitDoWhileStatement(final ASTNode node) {
  ...
  @Override public void visitForInStatement(final ASTNode node) {
  ...
  @Override public void visitWhileStatement(final ASTNode node) {
  ...
  @Override public void visitWithStatement(final ASTNode node) {
  ...
  @Override public void visitTryStatement(final ASTNode node) {
  ...
  @Override public void visitCatchBlock(final ASTNode node) {
    if (type1 == JSTokenTypes.LPAR || type2 == JSTokenTypes.RPAR) {
  ...
  @Override public void visitSwitchStatement(final ASTNode node) {
  ...
  @Override public void visitArgumentList(final ASTNode node) {
    if (type1 == JSTokenTypes.LPAR || type2 == JSTokenTypes.RPAR) {
      setSingleSpace(false);
    }
    else if (type1 == JSTokenTypes.COMMA) {
  ...
  @Override public void visitStatement(final ASTNode node) {
    if (type2 == JSTokenTypes.SEMICOLON) {
      setSingleSpace(false);
    }
  }

  @Override public void visitVarStatement(final ASTNode node) {
    if (type1 == JSTokenTypes.VAR_KEYWORD) {
      setSingleSpace(true);
    }
  }

  @Override public void visitVariable(final ASTNode node) {
    if (type1 == JSTokenTypes.EQ || type2 == JSTokenTypes.EQ) { // Initializer
      setSingleSpace(mySettings.SPACE_AROUND_ASSIGNMENT_OPERATORS);
    }
  }

  @Override public void visitBinaryExpression(final ASTNode node) {
  ...
  @Override public void visitConditionalExpression(final ASTNode node) {
