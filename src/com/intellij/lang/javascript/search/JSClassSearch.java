...
public abstract class JSClassSearch implements QueryExecutor<JSClass, JSClassSearch.SearchParameters> {
  ...
  // implementation
  private static final QueryFactory<JSClass, SearchParameters> INTERFACE_IMPLEMENTATIONS_QUERY_FACTORY = new QueryFactory<JSClass, SearchParameters>();
  ...
  static {
    INTERFACE_IMPLEMENTATIONS_QUERY_FACTORY.registerExecutor(new JSClassSearch() {
      ...
      public boolean execute(final SearchParameters queryParameters, Processor<JSClass> consumer) {
        ...
        if (queryParameters.isCheckDeepInheritance()) {
          ...
          consumer = new Processor<JSClass>() {
            public boolean process(JSClass jsClass) {
              return consumerCopy.process(jsClass) && OUR_CLASS_SEARCH_EXECUTOR.processDirectInheritors(jsClass, this, false, visited, queryParameters.getScope());
            }
          };
        }
        ...
        final boolean b =
          processDirectInheritors(queryParameters.getTargetClass(), consumerToUse, queryParameters.isCheckDeepInheritance(), visited, queryParameters.getScope());
        if(b) {
          return searchClassInheritors(queryParameters.getTargetClass(), queryParameters.isCheckDeepInheritance()).forEach(
      ...
      protected Collection<JSClass> getInheritors(JSClassInheritorsProvider provider, String parentName, Project project,
  ...
  public boolean execute(final SearchParameters queryParameters, final Processor<JSClass> consumer) {
    return processDirectInheritors(queryParameters.getTargetClass(), consumer, queryParameters.isCheckDeepInheritance(), null, queryParameters.getScope());
  }
      ...
      if (classes != null) {
        for(JSClass superClassCandidate: classes) {
          if (superClassCandidate.isEquivalentTo(superClass)) {
            ...
            if (checkDeep && !processDirectInheritors(candidate, consumer, checkDeep, processed, scope)) return false;
          }
        }
      }
    }
