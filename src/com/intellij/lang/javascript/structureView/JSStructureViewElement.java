...
public class JSStructureViewElement implements StructureViewTreeElement {
    ...
    if (element instanceof JSClass) {
      for(JSClass clazz:((JSClass)element).getSuperClasses()) {
        ...
        for(StructureViewTreeElement e:structureViewTreeElements) {
          if (!isVisible((PsiElement)e.getValue(), element)) continue;
          ...
          if (!(e instanceof JSStructureViewElement)) continue;
      ...
      final MyProcessor processor = new MyProcessor(index, visitedTypes) {
        ...
        protected boolean process(final PsiElement namedElement, final JSNamespace namespace) {
          ...
          if (referencedNamedIds.contains(elementProxy.getNameId()) ||
              !isVisible(namedElement, element)
             ) {
            return true;
          }
          referencedNamedIds.add(elementProxy.getNameId());
          final JSStructureViewElement jsStructureViewElement = createStructureViewElement(
      ...
      if (myProxy != null) {
        ...
        if (namespace != null && "Object".equals(namespace.getQualifiedName(index))) doIterateTypes = false;
      }
  ...
  protected List<StructureViewTreeElement> collectMyElements(final TIntHashSet referencedNamedIds, final JSIndexEntry entry,
                                                         final JSNamespace ns, final JavaScriptIndex index) {
  ...
  private static void collectChildrenFromElement(final PsiElement element, final TIntHashSet referencedNamedIds, final JavaScriptIndex index,
    ...
    element.acceptChildren(new JSElementVisitor() {
      ...
      @Override public void visitJSParameter(final JSParameter node) {
      ...
      public void visitJSIncludeDirective(final JSIncludeDirective includeDirective) {
        ...
        if (file instanceof JSFile) {
          if (visited != null && visited.contains(file)) return;
          if (visited == null) visited = new THashSet<PsiFile>();
          visited.add(file);
  ...
  public @Nullable PsiElement getUpToDateElement() {
    boolean isValid = myElement.isValid();
    
    if (!isValid && myProxy != null) {
      if (!myProxy.isValid()) return null;
  ...
  static abstract class JSStructureItemPresentationBase implements ItemPresentation {
    ...
    public TextAttributesKey getTextAttributesKey() {
      if (element.myInherited) return CodeInsightColors.NOT_USED_ELEMENT_ATTRIBUTES;
      if (element.getProxy() != null && element.getProxy().isDeprecated()) return CodeInsightColors.DEPRECATED_ATTRIBUTES;
      return null;
    }
