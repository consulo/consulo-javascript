...
class JavaScriptGenerateAccessorHandler extends BaseJSGenerateHandler {
  ...
  protected String getTitleKey() {
    return mode == GenerationMode.GETTERS ? "generate.getter.fields.chooser.title":mode == GenerationMode.SETTERS ? "generate.setter.fields.chooser.title":
        mode == GenerationMode.GETTERS_AND_SETTERS ? "generate.getter.setter.chooser.title":
          mode == GenerationMode.TOSTRING ? "generate.to.string.chooser.title":
  ...
  protected BaseCreateMethodsFix createFix(final JSClass jsClass) {
    if (mode == GenerationMode.GETTERS_AND_SETTERS) {
      return new BaseCreateMethodsFix<JSVariable>(jsClass) {
        private boolean toCreateBindableProperties =
          myCreateBindableProperties != null ? myCreateBindableProperties.isSelected(): ApplicationManager.getApplication().isUnitTestMode();
        final MyBaseCreateMethodsFix generateGetterFix = new MyBaseCreateMethodsFix(GenerationMode.GETTERS, jsClass,
                                                                                    toCreateBindableProperties);
        final MyBaseCreateMethodsFix generateSetterFix = new MyBaseCreateMethodsFix(GenerationMode.SETTERS, jsClass,
                                                                                    toCreateBindableProperties);
        ...
        public void invoke(@NotNull final Project project, final Editor editor, final PsiFile file) throws IncorrectOperationException {
          ...
          while(variableIterator.hasNext()) {
            ...
            if(JSResolveUtil.findParent(var) == jsClass) {
              ...
              if (hadSuperClassConstructorInitializationBefore) initialization += ", ";
              else initialization+="super(";
  ...
  protected void collectCandidates(final JSClass clazz, final Collection<JSNamedElementNode> candidates) {
    ...
    final ResolveProcessor processor = new ResolveProcessor(null) { {setToProcessMembers(true); setToProcessHierarchy(false); setLocalResolve(true);}
      @Override
  ...
  private static class MyBaseCreateMethodsFix extends BaseCreateMethodsFix<JSVariable> {
    ...
    @Override
      ...
      if(myMode == GenerationMode.GETTERS) return "get ";
      if(myMode == GenerationMode.SETTERS) return "set ";
      return super.buildFunctionKind(fun);
    }

    protected String buildReturnType(final String typeString) {
      if(myMode == GenerationMode.SETTERS) return "void";
  ...
  private static String transformVarNameToAccessorName(String s, final JSCodeStyleSettings codeStyleSettings) {
    if (StringUtil.startsWith(s, codeStyleSettings.FIELD_PREFIX)) s = s.substring(codeStyleSettings.FIELD_PREFIX.length());
