...
public abstract class BaseCreateMethodsFix<T extends JSNamedElement & JSAttributeListOwner> {
  ...
  protected void evalAnchor(final Editor editor, final PsiFile file) {
    ...
    if (at != null &&
      ...
      if (atNode.getElementType() == JSTokenTypes.RBRACE) return;
      
      for(ASTNode node= atNode; node != null; node = node.getTreeNext()) {
        if (node.getElementType() == JSTokenTypes.LBRACE) return;
      }
      anchor = at;
    }
  }

  protected PsiElement doAddOneMethod(final Project project, final String functionText, PsiElement anchor) throws IncorrectOperationException {
    if (functionText != null && functionText.length() > 0) {
      ...
      if (anchor != null && anchor.isValid()) {
        ...
        if (!(anchor instanceof PsiWhiteSpace) ||
           ( (anchorText = anchor.getText()).indexOf("<![CDATA[") == -1 &&
             anchorText.indexOf("]]>") == -1
           )
          ) {
          defaultAdd = false;
  ...
  protected @NonNls String buildFunctionKind(final T fun) {
    if (fun instanceof JSFunction) {
      final JSFunction function = (JSFunction)fun;
      if (function.isGetProperty()) return "get ";
      else if (function.isSetProperty()) return "set ";
    }
    return "";
  }

  protected @NonNls String buildFunctionBodyText(@NonNls String retType, final JSParameterList parameterList, final T func) {
    return " {}";
  }

  protected @NonNls String buildFunctionAttrText(@NonNls String attrText, final JSAttributeList attributeList, final T function) {
  ...
  public void addElementsToProcessFrom(final Collection<JSNamedElementNode> selectedElements) {
    for(JSNamedElementNode el:selectedElements) addElementToProcess((T)el.getPsiElement());
  }
