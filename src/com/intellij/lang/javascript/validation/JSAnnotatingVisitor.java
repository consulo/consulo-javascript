...
public class JSAnnotatingVisitor extends JSElementVisitor implements Annotator {
  ...
  @Override public void visitJSAttributeNameValuePair(final JSAttributeNameValuePair attributeNameValuePair) {
    checkReferences(attributeNameValuePair, ProblemKind.ERROR);
  }

  @Override public void visitJSIncludeDirective(final JSIncludeDirective includeDirective) {
  ...
  private void checkReferences(final PsiElement includeDirective, ProblemKind kind) {
    for(PsiReference ref:includeDirective.getReferences()) {
      if (!ref.isSoft() && hasBadResolve(ref)) {
        ...
        Annotation annotation = kind == ProblemKind.ERROR ? myHolder.createErrorAnnotation(range, message):
                                kind == ProblemKind.WARNING ? myHolder.createWarningAnnotation(range, message):
  ...
  public void visitJSReferenceList(final JSReferenceList referenceList) {
    ...
    if (withinImplements && jsClass.isInterface()) {
      myHolder.createErrorAnnotation(referenceList, JSBundle.message("javascript.validation.message.implements.for.interface.not.allowed"));
      return;
    }
    ...
    if (referenceExpressions != null) {
      for(JSReferenceExpression expr: referenceExpressions) {
        ...
        if (s != null) nameToExprMap.put(s, expr);
      }
    }
    ...
    for(JSClass clazz:referenceList.getReferencedClasses()) {
      ...
      if (!b && withinImplements) {
        myHolder.createErrorAnnotation(expr, JSBundle.message("javascript.validation.message.interface.name.expected.here"));
      }
      else if (withinExtends && b != jsClass.isInterface()) {
        myHolder.createErrorAnnotation(expr, JSBundle.message(
            !b ? "javascript.validation.message.interface.name.expected.here" : "javascript.validation.message.class.name.expected.here"));
      }
      if (clazz == jsClass) {
        myHolder.createErrorAnnotation(expr, JSBundle.message("javascript.validation.message.circular.dependency")
  ...
  public interface ErrorReportingClient {
    enum ProblemKind { ERROR, WARNING }
  ...
  public static void checkImplementedMethods(final JSClass jsClass, final ErrorReportingClient reportingClient) {
    final JSResolveUtil.CollectMethodsToImplementProcessor implementedMethodProcessor = new ImplementedMethodProcessor(jsClass) {
      ...
      protected void addNonimplementedFunction(final JSFunction function) {
        ...
        reportingClient.reportError(node, JSBundle.message("javascript.validation.message.interface.method.not.implemented",
                                                           function.getName(), ((JSClass)JSResolveUtil.findParent(function)).getQualifiedName()),
      ...
      protected void addImplementedFunction(final JSFunction interfaceFunction, final JSFunction implementationFunction) {
        ...
        if (incompatibleSignature != SignatureMatchResult.COMPATIBLE_SIGNATURE) {
          ...
          if (incompatibleSignature == SignatureMatchResult.PARAMETERS_DIFFERS) {
            ...
            reportingClient.reportError(implementationReturnTypeExpr != null ? implementationReturnTypeExpr.getNode():implementationFunction.findNameIdentifier(),
                                        JSBundle.message(
  ...
  private static ASTNode findElementForAccessModifierError(final @NotNull JSFunction o, final JSAttributeList attributeList) {
    if (attributeList != null) {
      ...
      if (accessTypeElement != null) return accessTypeElement.getNode();
    }
  ...
  public void visitJSAttributeList(JSAttributeList attributeList) {
    ...
    if (namespaceOrAccessModifierElement == null) {
      ...
      myHolder.createErrorAnnotation(namespaceOrAccessModifierElement,
        ...
        .registerFix(
          new RemoveASTNodeFix(namespaceOrAccessModifierElement.getNode(), "javascript.fix.remove.namespace.reference"));
    ...
    if (namespaceOrAccessModifierElement != null) {
      ...
      if (!(parentForCheckingNsOrAccessModifier instanceof JSClass)) {
        ...
        if (((!(nodeUnderPackage = (parentForCheckingNsOrAccessModifier instanceof JSPackageStatement)) &&
          ...
          myHolder.createErrorAnnotation(namespaceOrAccessModifierElement, JSBundle.message(nodeUnderPackage
            ...
            .registerFix(new RemoveASTNodeFix(namespaceOrAccessModifierElement.getNode(), nsRef
        ...
        if (parent instanceof JSPackageStatement) {
          ...
          if (function.isConstructor()) {
            ...
            if (typeString != null && !"void".equals(typeString)) {
              // TODO: fix!
              myHolder.createErrorAnnotation(function.getReturnTypeElement(), JSBundle.message("javascript.validation.message.set.method.should.be.void.or.without.type"));
            }

            JSParameterList parameterList = function.getParameterList();
            if (parameterList != null && parameterList.getParameters().length != 1) {
              // TODO: fix!
              myHolder.createErrorAnnotation(parameterList, JSBundle.message("javascript.validation.message.set.method.should.have.one.parameter"));
            }
            ...
            if (typeString == null || "void".equals(typeString)) {
              ...
              myHolder.createErrorAnnotation(
                ...
                JSBundle.message("javascript.validation.message.get.method.should.be.valid.type", typeString != null ? typeString:"empty"));
            }

            JSParameterList parameterList = function.getParameterList();
            if (parameterList != null && parameterList.getParameters().length != 0) {
              // TODO: fix!
              myHolder.createErrorAnnotation(parameterList, JSBundle.message("javascript.validation.message.get.method.should.have.no.parameter"));
            }
          }
        }
  ...
  private void checkMissedSuperCall(JSReferenceExpression node, JSFunction constructor, JSClass jsClass) {
    ...
    if (nontrivialSuperClassConstructor != null) {
      ...
      if (!hasSuperConstructorCall(constructor)) {
        annotation = myHolder.createErrorAnnotation(node, JSBundle.message("javascript.validation.message.missed.super.constructor.call"));
      }
  ...
  private boolean hasSuperConstructorCall(JSFunction jsFunction) {
    if (jsFunction == null) return false;
  ...
  public void visitJSParameterList(JSParameterList node) {
    ...
    for(JSParameter parameter:node.getParameters()) {
      ...
      if (hasInitializer && !initializerPresent) {
        ...
        myHolder.createErrorAnnotation(
          parameter,
          JSBundle.message("javascript.validation.message.parameter.should.be.initialized")
        ...
        myHolder.createErrorAnnotation(
          parameter,
          JSBundle.message("javascript.validation.message.rest.parameter.should.not.be.initialized")
      ...
      if (parameter.isRest() && !foundRest) {
        ...
        if (typeElement != null && !"Array".equals(typeElement.getText())) {
          ...
          ).registerFix(new RemoveASTNodeFix("javascript.fix.remove.type.reference", getNodesBefore(typeElement, JSTokenTypes.COLON)));
        }
      } else if (foundRest) {
        myHolder.createErrorAnnotation(
          parameter,
          JSBundle.message("javascript.validation.message.parameter.is.not.allowed.after.rest.parameter")
        ).registerFix(new RemoveASTNodeFix(parameter.getNode(), "javascript.fix.remove.parameter"));
      }
    }
  }
  ...
  @Override public void visitJSAssignmentExpression(final JSAssignmentExpression expression) {
  ...
  public void visitJSArrayLiteralExpression(final JSArrayLiteralExpression node) {
    ...
    if (child instanceof PsiWhiteSpace) child = child.getPrevSibling();
    ASTNode childNode;

    if (child != null && (childNode = child.getNode()) != null && childNode.getElementType() == JSTokenTypes.COMMA) {
      final Annotation annotation =
        myHolder.createWarningAnnotation(child, JSBundle.message("javascript.validation.message.unneeded.comma"));
    ...
    if (blocks.length > 1) {
      ...
      for(JSCatchBlock block:blocks) {
        ...
        if (typeToCatch.contains(s)) {
          final Annotation annotation =
            myHolder.createErrorAnnotation(block, JSBundle.message("javascript.validation.message.duplicated.catch.block"));
  ...
  @Override public void visitJSContinueStatement(final JSContinueStatement node) {
  ...
  @Override public void visitJSBreakStatement(final JSBreakStatement node) {
  ...
  public void visitJSFunctionDeclaration(final JSFunction node) {
    ...
    if (parent instanceof JSFile) {
      ...
      if (parent instanceof JSClass && node.getName().equals(((JSClass)parent).getName()) && JavaScriptSupportLoader.isFlexMxmFile(parent.getContainingFile())) {
        ...
        if (!b && hasOverride) {
          ...
          if ((overrideAttrList == null &&
            ...
            final Annotation annotation1 = myHolder.createErrorAnnotation(
              ...
              JSBundle.message("javascript.validation.message.function.override.incompatible.access.modifier",
                overrideNs != null ?
                overrideNs:
                (overrideAttrList != null ?
                 overrideAttrList.getAccessType().toString() :
                 JSAttributeList.AccessType.PACKAGE_LOCAL.toString()).toLowerCase()
              )
            );
          ...
          if (incompatibleSignature == SignatureMatchResult.PARAMETERS_DIFFERS) {
            ...
            final Annotation annotation = myHolder.createErrorAnnotation(
                returnTypeExpr != null ? returnTypeExpr.getNode(): node.findNameIdentifier(),
                JSBundle.message("javascript.validation.message.function.override.incompatible.signature2",
  ...
  private static SignatureMatchResult checkCompatibleSignature(final JSFunction fun, final JSFunction override) {
    ...
    if (result == SignatureMatchResult.COMPATIBLE_SIGNATURE) {
      for (int i = 0; i < parameters.length; ++i) {
        if (!compatibleType(overrideParameters[i].getTypeString(), parameters[i].getTypeString(), overrideParameterList, nodeParameterList) ||
  ...
  @Override public void visitJSReturnStatement(final JSReturnStatement node) {
    ...
    if (element instanceof JSFunction) {
      ...
      if (typeString != null && !"void".equals(typeString) && node.getExpression() == null) {
        myHolder.createErrorAnnotation(node, JSBundle.message("javascript.validation.message.return.value.of.type.is.required", typeString));
      }
    }
  }

  @Override public void visitJSLabeledStatement(final JSLabeledStatement node) {
  ...
  private void checkPackageStatement(final JSPackageStatement packageStatement) {
    ...
    if (elements.size() > 1) {
      for(JSNamedElement el:elements) {
        ...
        ).registerFix(new RemoveASTNodeFix(el.getNode(), "javascript.fix.remove.externally.visible.symbol"));
      }
    }
  ...
  public static class RemoveASTNodeFix implements IntentionAction, LocalQuickFix {
    ...
    public RemoveASTNodeFix(@PropertyKey(resourceBundle = JSBundle.BUNDLE) String propKey, final ASTNode... astNodes) {
      myPropKey = propKey;
      myAstNodes = astNodes;
    }
    ...
    public boolean isAvailable(@NotNull final Project project, final Editor editor, final PsiFile file) {
      for(ASTNode astNode:myAstNodes) {
        if (!astNode.getPsi().isValid()) return false;
      }
    ...
    public void invoke(@NotNull final Project project, final Editor editor, final PsiFile file) throws IncorrectOperationException {
      if (!CodeInsightUtilBase.getInstance().prepareFileForWrite(file)) return;
      for(ASTNode astNode:myAstNodes) {
        if (astNode.getPsi().isValid()) astNode.getPsi().delete();
      }
    }
  ...
  private static class AddOverrideIntentionAction implements IntentionAction {
    ...
    public void invoke(@NotNull final Project project, final Editor editor, final PsiFile file) throws IncorrectOperationException {
      ...
      else myNode.addBefore(createdAttrList, myNode.getFirstChild());
    }
  ...
  private class SimpleErrorReportingClient implements ErrorReportingClient {
    public void reportError(final ASTNode nameIdentifier, final String s, ProblemKind kind, final IntentionAction implementMethodsFix) {
      final Annotation annotation = kind == ProblemKind.ERROR ? myHolder.createErrorAnnotation(nameIdentifier, s):myHolder.createWarningAnnotation(nameIdentifier, s);
      if (implementMethodsFix != null) annotation.registerFix(implementMethodsFix);
    }
  }
