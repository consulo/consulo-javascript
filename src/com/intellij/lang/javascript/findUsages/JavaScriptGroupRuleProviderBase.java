    ...
    return new UsageGroupingRule() {
      ...
      public UsageGroup groupUsage(final Usage usage) {
        if (usage instanceof PsiElementUsage) {
          ...
          if (!psiElement.getLanguage().isKindOf(JavaScriptSupportLoader.JAVASCRIPT.getLanguage())) return null;
          final JSNamedElement element = PsiTreeUtil.getParentOfType(psiElement, getUsageClass());

          if (isAcceptableElement(element)) { return createUsageGroup((T)element); }
        }
        return null;
      }
    };
