...
public class JSUnusedLocalSymbolsInspection extends JSInspection {
  ...
  @NonNls public static final String SHORT_NAME = "JSUnusedLocalSymbols";
  ...
  protected JSElementVisitor createVisitor(final ProblemsHolder holder) {
    return new JSElementVisitor() {
      ...
      @Override public void visitJSParameterList(final JSParameterList node) {
      ...
      @Override public void visitJSParameter(final JSParameter node) {
        ...
        if (scopeNode == null || scopeNode instanceof JSCatchBlock) return;
        // TODO: calculate more accurately right here (we depend in this place for slow marker pass to finish before)
        if(scopeNode.getUserData(JavaScriptLineMarkerProvider.ourParticipatesInHierarchyKey) != null) return;
        ...
        if (p instanceof JSParameter) {
          ...
          if (ecma && node instanceof JSFunctionExpression) {
            ...
            if (params.length == 1) {
              ...
              if (type != null) type = JSImportHandlingUtil.resolveTypeName(type, p);
          ...
          if (lastUsedParameterIndex == nonCounted) {
            ...
            for(PsiElement param:unusedDeclarationsSet) {
              if (!(param instanceof JSParameter)) continue;
