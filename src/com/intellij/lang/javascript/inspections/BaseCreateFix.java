...
abstract class BaseCreateFix implements LocalQuickFix {
  ...
  public void applyFix(@NotNull Project project, @NotNull ProblemDescriptor descriptor) {
    ...
    if (qualifier != null && ecma) {
      ...
      if (type == null) return; // can not resolve

      if (type != null && type.isWritable()) {
        if (type instanceof XmlBackedJSClassImpl) type = type.getParent().getContainingFile();
      ...
      if (ecma) {
        if(qualifier != null) {
          if(qualifier instanceof JSReferenceExpression) {
            ...
            if (qualifierResolve instanceof JSClass ||
                qualifierResolve instanceof XmlFile
                ) {
              isStatic = true;
            }
          }
  ...
  public static @Nullable Editor getEditor(final Project project, final PsiFile realFile) {
    if ( !CodeInsightUtilBase.getInstance().prepareFileForWrite(realFile)) return null;
  ...
  protected static JSExpression addAccessModifier(final Template template, final JSReferenceExpression referenceExpression, final boolean ecma,
    ...
    if (ecma) {
      if ((qualifier == null || qualifier instanceof JSThisExpression)) template.addTextSegment("private ");
      if (staticContext) template.addTextSegment("static ");
    }
    return qualifier;
  }
  ...
  protected void addSemicolonSegment(final Template template, final PsiFile file) {
    ...
    if (semicolon.length() > 0) template.addTextSegment(semicolon);
  }

  static void guessExprTypeAndAddSuchVariable(final JSExpression passedParameterValue, final Template template, final String var1, final PsiFile file,
                                               final boolean ecma) {
  ...
  protected static JSClass findClass(PsiFile file, final PsiElement anchorParent) {
    if(anchorParent instanceof JSClass) return (JSClass)anchorParent;
