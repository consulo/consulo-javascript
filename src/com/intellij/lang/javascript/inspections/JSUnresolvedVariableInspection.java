...
public class JSUnresolvedVariableInspection extends JSInspection {
            ...
            if (nameIdentifier != null) {
              ...
              if (myOnTheFly) {
                ...
                if ((qualifier == null ||
                     ...
                     qualifier instanceof JSThisExpression
                    ) &&
                  ...
                  if (!JSResolveUtil.isExprInTypeContext(node)) {
                    if (node.getParent() instanceof JSArgumentList) {
                      fixes.add(new JSUnresolvedFunctionInspection.CreateJSFunctionIntentionAction(referencedName, !(qualifier == null || (qualifier instanceof JSThisExpression && ecma))) {
                        @Override
                        protected void addParameters(Template template,
                          ...
                          if (method instanceof JSReferenceExpression && "bindSetter".equals(((JSReferenceExpression)method).getReferencedName())) {
                            ...
                            if (ecma) template.addTextSegment(":int");
                          }
                        }
                    ...
                    if (suggestCreateVar) {
                      fixes.add(
                        new CreateJSVariableIntentionAction( referencedName, isField, false )
                      );

                      if (ecma) {
                        fixes.add(
                          new CreateJSVariableIntentionAction( referencedName, isField, true )
                        );
                      }
                    }
  ...
  private static class CreateJSVariableIntentionAction extends BaseCreateJSVariableIntentionAction {
    ...
    @NonNls private static final String CONSTANT_STATEMENT_START = "const ";
    ...
    public String getName() {
      return JSBundle.message(
        isField ?
        isConstant ? "javascript.create.constant.field.intention.name":"javascript.create.property.intention.name":
        isConstant ? "javascript.create.constant.intention.name":"javascript.create.variable.intention.name",
        myReferencedName
      );
    }
    ...
    protected void buildTemplate(final Template template, final JSReferenceExpression referenceExpression, final boolean ecma, boolean staticContext,
      ...
      if (qualifier == null || ecma) template.addTextSegment(isConstant ? CONSTANT_STATEMENT_START:VAR_STATEMENT_START);
  ...
  private static class CreateJSPropertyAccessorIntentionAction extends JSUnresolvedFunctionInspection.CreateJSFunctionIntentionActionBase {
    ...
    protected void addBody(Template template, JSReferenceExpression refExpr, PsiFile file) {
      ...
      JSCodeStyleSettings settings = CodeStyleSettingsManager.getInstance(file.getProject()).getCurrentSettings().getCustomSettings(JSCodeStyleSettings.class);
