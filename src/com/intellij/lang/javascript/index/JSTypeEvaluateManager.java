...
public class JSTypeEvaluateManager implements ProjectComponent {
  ...
  public boolean iterateTypeHierarchy(String fqTypeName, NamespaceProcessor processor) {
    if (fqTypeName == null || fqTypeName.length() == 0) return true;
    boolean result = true;
  ...
  private boolean doIterateTypeImpl(final JSNamespace namespace, final NamespaceProcessor processor, TIntHashSet visited) {
    ...
    if (superNameId == 0) {
      if (namespace.getNameId() != myObjectNameIndex ||
          !namespace.getQualifiedName(myIndex).equals(JSResolveUtil.OBJECT_CLASS_NAME)) {
    ...
    if (superNameId != 0 && !visited.contains(superNameId)) {
      ...
      if (superNamedElements == null) {
        ...
        if (aPackage != null) superNamedElements = aPackage.getInstances();
        else {
          return true;
        }
      }
  ...
  public static String getComponentType(String s) {
    if (s.endsWith("[]")) return s.substring(0,s.length() - 2);
      ...
      if (i != -1) return s.substring(i+1);
        ...
        if (i2 == -1) i2 = s.length();
        if (i != -1 && i2 != -1 && i2 > i) return s.substring(i+1,i2);
      }
    }
    return s;
  }
  ...
  public static String getInstanceNameByType(String className) {
    ...
    if ("Element".equals(className)) return BaseJSSymbolProcessor.HTML_ELEMENT_TYPE_NAME;
    if ("DOMNode".equalsIgnoreCase(className)) return BaseJSSymbolProcessor.HTML_ELEMENT_TYPE_NAME;
    return className;
  }
  ...
  private void remove(final JSNamespace el) {
    ...
    if (list != null) {
      ...
      list.remove(el);
      if (list.size() == 0) myName2ElementMap.remove(ourId);
      else myName2ElementMap.put(ourId, list);
    }
  }
  ...
  public String getBaseType(final JSNamespace namespace) {
    synchronized (myIndex) {
      ...
      if (i == 0) return JSResolveUtil.OBJECT_CLASS_NAME;
      return myIndex.getStringByIndex(i);
    }
  }
  ...
  private void doAddNsWithType(final int key, final JSNamespace superNs) {
    ...
    if (namespaces == null) {
      ...
      if (namespaces.indexOf(superNs) == -1) { // copy on write
    ...
    synchronized (myIndex) {
      ...
      return myNsToSuperMap.forEachEntry(new TObjectIntProcedure<JSNamespace>() {
        ...
        public boolean execute(final JSNamespace a, final int b) {
          if (nameIndex == b) return processor.process(a);
          return true;
        }
      });
    }
