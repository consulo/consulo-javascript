...
final class MyJSNamedItem extends PsiElementBase implements JSNamedElementProxy, ModificationTracker {
  ...
  MyJSNamedItem(final JSIndexEntry jsIndexEntry, int _offset, int _nameId, NamedItemType _type) {
    ...
    myOffset = _offset;
    myNameId = _nameId;
    final int valueTag = _type == NamedItemType.Definition
                         ? DEFINITION_TAG
                         : _type == NamedItemType.Function
                           ? FUNCTION_TAG
                           : _type == NamedItemType.FunctionExpression
                             ? FUNCTION_EXPRESSION_TAG
                             : _type == NamedItemType.FunctionProperty
                               ? FUNCTION_PROPERTY_TAG
                               : _type == NamedItemType.Property
                                 ? PROPERTY_TAG
                                 : _type == NamedItemType.Variable ? VARIABLE_TAG :
                                    _type == NamedItemType.MemberVariable ? MEMBER_VARIABLE_TAG :
                                      _type == NamedItemType.MemberFunction ? MEMBER_FUNCTION_TAG :
                                        _type == NamedItemType.Clazz ? CLASS_TAG :
                                          _type == NamedItemType.Namespace ? NAMESPACE_TAG :
                                            _type == NamedItemType.ImplicitFunction ? IMPLICIT_FUNCTION_TAG :
                                              _type == NamedItemType.ImplicitVariable ? IMPLICIT_VARIABLE_TAG :
                                                _type == NamedItemType.AttributeValue ? ATTR_VALUE_TAG :
                                          UNDEFINED_TAG;
  ...
  public boolean equals(final Object obj) {
    ...
    if (!(obj instanceof MyJSNamedItem)) return false;
  ...
  public void write(SerializationContext context) throws IOException {
    ...
    final int browserSpecific =
      (context.browserSupportManager.isGeckoSpecificSymbol(this) ? GECKO_SPECIFIC_ITEM_TAG:
        context.browserSupportManager.isIESpecificSymbol(this) ? IE_SPECIFIC_ITEM_TAG:
          context.browserSupportManager.isOperaSpecificSymbol(this) ? OPERA_SPECIFIC_ITEM_TAG:0
      ) << BROWSER_TAG_SHIFT;
  ...
  public PsiElement getNavigationElement() {
    final PsiElement element = getElement();
    if (element instanceof XmlToken) return element.getParent();
    return element;
  }
  ...
  public PsiElement getElement() {
    ...
    if (element == null || !element.isValid()) {
      ...
      if (element == null) {
        ...
        if (valueType != NamedItemType.AttributeValue && elementAt != null) {
          ...
          if (valueType == NamedItemType.FunctionExpression && element instanceof JSDefinitionExpression) {
            ...
            if (element instanceof JSNewExpression) element = ((JSNewExpression)element).getMethodExpression();
      ...
      if (element == null) element = PsiUtilBase.NULL_PSI_ELEMENT;
  ...
  private Icon getIconBase() {
    ...
    if (valueType == NamedItemType.Function ||
      ...
      if (valueType == NamedItemType.MemberFunction  &&
          ( hasProperty(Property.GetFunction) ||
  ...
  public NamedItemType getType() {
    ...
    final NamedItemType valueType =
      valueTag == DEFINITION_TAG ? NamedItemType.Definition:
        valueTag == FUNCTION_TAG ? NamedItemType.Function:
          valueTag == FUNCTION_EXPRESSION_TAG ? NamedItemType.FunctionExpression:
            valueTag == FUNCTION_PROPERTY_TAG ? NamedItemType.FunctionProperty:
              valueTag == MEMBER_FUNCTION_TAG ? NamedItemType.MemberFunction:
                valueTag == PROPERTY_TAG ? NamedItemType.Property:
                  valueTag == VARIABLE_TAG ? NamedItemType.Variable:
                    valueTag == MEMBER_VARIABLE_TAG ? NamedItemType.MemberVariable:
                      valueTag == CLASS_TAG ? NamedItemType.Clazz:
                        valueTag == NAMESPACE_TAG ? NamedItemType.Namespace:
                          valueTag == IMPLICIT_FUNCTION_TAG ? NamedItemType.ImplicitFunction:
                            valueTag == IMPLICIT_VARIABLE_TAG ? NamedItemType.ImplicitVariable:
                              valueTag == ATTR_VALUE_TAG ? NamedItemType.AttributeValue:
                  null;
  ...
  private static int getPropertyShift(final Property property) {
    ...
    if (shift == -1) throw new IllegalArgumentException("Illegal property passed:"+property);
    return shift;
  }
