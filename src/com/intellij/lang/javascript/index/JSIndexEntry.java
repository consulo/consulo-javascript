...
public class JSIndexEntry {
  ...
  private final static ProxyProcessor<Set<NavigationItem>> myFindClassesByName = new ProxyProcessor<Set<NavigationItem>>() {
    ...
    public void process(final JSNamedElementProxy proxy, final JavaScriptIndex index, final Set<NavigationItem> param) {
      if (proxy.getType() == NamedItemType.Clazz) param.add( proxy );
    }
  };
  ...
  private static final ProxyProcessor<Set<String>> myCollectSymbolNamesProcessor = new ProxyProcessor<Set<String>>() {
    ...
    public void process(final JSNamedElementProxy proxy, final JavaScriptIndex index, final Set<String> param) {
      if (proxy.getType() != NamedItemType.Clazz) param.add(index.getStringByIndex(proxy.getNameId()));
    }
  };
  ...
  public void fillClassByName(final String name, final Set<NavigationItem> classes) {
    process(
      myIndex.getIndexOf( name ), myFindClassesByName,
      classes
    );
  }
  ...
  private void doInitFor(final IndexEntryContent content) {
    ...
    myIndexValue = CachedValuesManager.getManager(myIndex.getProject()).createCachedValue(new CachedValueProvider<IndexEntryContent>() {
      ...
      public Result<IndexEntryContent> compute() {
        ...
        if(psiFile == null) { // virtual file become invalid?
        ...
        synchronized(myIndex) {
          if (myContent == null) {
            ...
            if (!myContentBelongsOnlyToMyFile) invalidate();
      ...
      private void updateFromTree(final PsiFile psiFile) {
        JSSymbolUtil.visitSymbols(psiFile, myContent.myNamespace, new JSSymbolUtil.JavaScriptSymbolProcessorEx() {
          ...
          public boolean processFunction(JSNamespace namespace, final int nameId, JSNamedElement function) {
            ...
            addSymbol(
              nameId,
              function,
              parent instanceof JSClass ? NamedItemType.MemberFunction :
          ...
          public boolean processNamespace(final JSNamespace namespace, final int nameId, final JSNamedElement ns) {
            addSymbol(
              nameId,
              ns,
              NamedItemType.Namespace,
              namespace
            );
            return true;
          }

          public boolean processImplicitNamespace(final JSNamespace namespace, final int nameId, final PsiElement refExpr,
          ...
          public boolean processImplicitFunction(final JSNamespace namespace, final int nameId, final PsiElement refExpr) {
            addSymbol(
              nameId,
              refExpr,
              NamedItemType.ImplicitFunction,
              namespace
            );
            return true;
          }
          ...
          public boolean processImplicitVariable(final JSNamespace namespace, final int nameId, final PsiElement refExpr) {
            addSymbol(
              nameId,
              refExpr,
              NamedItemType.ImplicitVariable,
              namespace
            );
            return true;
          }
          ...
          private final void addSymbol(final int nameId, final PsiElement element,
                                 NamedItemType type, JSNamespace namespace, boolean finalReference) {
            if (nameId == -1) return;
            ...
            if ((type == NamedItemType.MemberFunction ||
                 ...
                 type == NamedItemType.FunctionProperty
                ) && element instanceof JSFunction) {
              final JSFunction function = (JSFunction)element;
              String s = function.getReturnTypeString();

              if (function.isGetProperty()) proxy.setProperty(JSNamedElementProxy.Property.GetFunction, true);
              if (function.isSetProperty()) proxy.setProperty(JSNamedElementProxy.Property.SetFunction, true);
              if (function.isConstructor()) {
                proxy.setProperty(JSNamedElementProxy.Property.Constructor, true);
                s = null;
              }

              if (s != null && s.length() > 0) typeEvaluateManager.setElementType(myElement, s);

              saveModifiersFromAttrList(proxy, function);
            } else if ((type == NamedItemType.Variable ||
                 type == NamedItemType.MemberVariable
            ...
            } else if (type == NamedItemType.Definition && element instanceof JSDefinitionExpression ||
                       type == NamedItemType.ImplicitFunction
                      ) {
              ...
              if (clazz.isInterface()) proxy.setProperty(JSNamedElementProxy.Property.Interface, true);
              if (clazz.findFunctionByName(clazz.getName()) != null) proxy.setProperty(JSNamedElementProxy.Property.HasConstructor, true);
            }
          ...
          public boolean processVariable(JSNamespace namespace, final int nameId, JSNamedElement variable) {
            ...
            addSymbol(
              nameId,
              variable,
              parent instanceof JSClass ? NamedItemType.MemberVariable: NamedItemType.Variable,
              namespace
            );
            return true;
          }
        ...
        if (!myContentBelongsOnlyToMyFile) myContent.myNamespace.validate();
      }
    }, false);

    if (content != null) myIndexValue.getValue();
  }
  ...
  private <T> void process(int requiredFieldId, ProxyProcessor<T> processor, T param) {
    ...
    if (myPsiFile == null || !processor.accepts(myPsiFile)) return;
  ...
  void fillSymbolsByName(final String name, final Set<NavigationItem> symbolNavItems) {
    process(
      myIndex.getIndexOf( name ), myFindSymbolsByNameProcessor,
      symbolNavItems
    );
  }
  ...
  void processSymbolsNoLock(JavaScriptSymbolProcessor processor) {
    ...
    if (!processor.acceptsFile(psiFile)) return;
    ...
    if (requiredNameId != -1) {
      ...
      if (value instanceof Object[]) {
        for(Object item:(Object[])value) {
          if (!dispatchProcessorCall(index, (JSNamedElementProxy)item, processor)) return;
        }
      } else if (value != null) {
        if (!dispatchProcessorCall(index, (JSNamedElementProxy)value, processor)) return;
      }
  ...
  private static boolean dispatchProcessorCall(final IndexEntryContent index,
    ...
    if (itemType == NamedItemType.Variable ||
        itemType == NamedItemType.MemberVariable
       ) {
  ...
  public final @Nullable PsiFile getFile() {
  ...
  public boolean processSymbolsInNs(final @NotNull JavaScriptSymbolProcessor myProcessor, final @NotNull JSNamespace jsNamespace) {
    PsiFile myPsiFile = getFile();
    if (myPsiFile == null) return true;
    if (!myProcessor.acceptsFile(myPsiFile)) return true;
      ...
      if (requiredNameId == -1) {
        for(Map.Entry<JSNamedElement,JSNamespace> e: index.mySymbolNameComponents.entrySet()) {
          if (e.getValue() == jsNamespace) {
            if(!dispatchProcessorCall(index, (JSNamedElementProxy)e.getKey(), myProcessor)) return false;
          }
        }
      } else {
        ...
        if (o instanceof Object[]) {
          for(Object o2:(Object[])o) {
            ...
            if (o2Ns == jsNamespace) {
              if (!dispatchProcessorCall(index, (JSNamedElementProxy)o2, myProcessor)) return false;
            }
          }
        } else {
          final JSNamespace oNs = o != null ? index.mySymbolNameComponents.get(o):null;
          if (oNs == jsNamespace) {
            if (!dispatchProcessorCall(index, (JSNamedElementProxy)o, myProcessor)) return false;
          }
        }
      }
