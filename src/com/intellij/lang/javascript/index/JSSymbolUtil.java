...
public class JSSymbolUtil {
  ...
  @NonNls private static final String EXTEND_PROPERTY_NAME = "extend";
  @NonNls private static final String MEMBERS_PROPERTY_NAME = "members";
  @NonNls private static final String STATICS_PROPERTY_NAME = "statics";
  @NonNls private static final String J_QUERY_VAR_NAME = "jQuery";
  @NonNls private static final String FN_FUN_NAME = "fn";
  ...
  private static JSElement findNameComponent(JSElement expr) {
    if (expr instanceof JSReferenceExpression) return expr;
    ...
    while(expr != null) {
      if (expr instanceof JSReferenceExpression) {
        ...
        if (method instanceof JSReferenceExpression) return method;
      } else {
        current = expr;
      }
                                                                           ...
                                                                           JavaScriptSymbolProcessingHost
  {
    ...
    private static final @NonNls String YAHOO_NAME = "YAHOO";
    private PsiFile myFile;
    @NonNls private static final String DOJO_NAME = "dojo";
    ...
    public JSSymbolVisitor(JSNamespace namespace, final JavaScriptSymbolProcessorEx symbolVisitor, PsiFile file) {
      ...
      myNamedTagsAreMembersOfDocument = !JavaScriptSupportLoader.isFlexMxmFile(myFile) && 
                                        (!(myFile instanceof XmlFile) || !XmlBackedJSClassImpl.doProcessAllTags((XmlFile)myFile));
    }
    ...
    public void processExpression(JSReferenceExpression expr) {
      if (expr != topExpression) {
        ...
        if (myFunction != null && expr.getQualifier() == null) {
          ...
          if(element != null) {
            if (!(element instanceof JSParameter)) reference = ""; // members derived from local vars do not go into indexing
      ...
      if (nameNode != null) {
        ...
        try {
          ...
          if (parentElement instanceof PsiFile ||
            ...
            if(!updateNsFromAttributeList(node)) return;
    ...
    @Override public void visitJSVarStatement(final JSVarStatement node) {
    ...
    private void processReferenceExpression(JSReferenceExpression element, JSExpression rOperand, JSNamespace suggestedNs) {
      ...
      try {
        ...
        if ((qualifier == null && myFunction != null) ||
            (qualifier instanceof JSThisExpression && myClazz != null)
            ) {
          ...
          JSResolveUtil.treeWalkUp(processor,
            element,
            element.getParent(),
            element
          );
    ...
    @Override public void visitJSWithStatement(final JSWithStatement node) {
    ...
    @Override public void visitJSObjectLiteralExpression(final JSObjectLiteralExpression node) {
      ...
      if (qualifyingExpression != null && qualifyingExpression.getParent() instanceof JSArrayLiteralExpression) {
        for(JSExpression expr:((JSArrayLiteralExpression)qualifyingExpression.getParent()).getExpressions()) {
          if (expr.getClass() != qualifyingExpression.getClass() || expr == qualifyingExpression) continue;
    ...
    @Override public void visitJSProperty(final JSProperty node) {
      ...
      IElementType type;
      boolean toProcessProperty = (nameIdentifier != null ?
                                  ((type = nameIdentifier.getElementType()) != JSTokenTypes.NUMERIC_LITERAL &&
                                   (type != JSTokenTypes.STRING_LITERAL || isIdentifier(name))):false);
      final int nameId = myIndex.getIndexOf(name);

      if (value instanceof JSFunction) {
        final JSFunction function = (JSFunction)value;
        if (toProcessProperty) mySymbolVisitor.processFunction(myNamespace, nameId, function);
          ...
          if ((EXTEND_PROPERTY_NAME.equals(name) || "Extends".equals(name) || "Implements".equals(name)) && 
            ...
            if (qualifyingExpression != null) {
              ...
              myTypeEvaluateManager.setBaseType(
                myNamespace,
                JSResolveUtil.ContextResolver.getQualifierOfExprAsString(qualifyingExpression), value.getText()
              );
            }
          }
        ...
        if (toProcessProperty) mySymbolVisitor.processProperty( myNamespace, nameId, node );
      }
    }
    ...
    @Override public void visitJSCallExpression(final JSCallExpression node) {
      ...
      if (methodExpression instanceof JSReferenceExpression &&
            ...
            for(JSExpression expr: jsExpressions) {
              if (expr instanceof JSThisExpression) {
                ...
                myTypeEvaluateManager.setBaseType(myThisNamespace,
                  myFunction.getName(), qualifier.getText()
                );
                break;
              }
            }
          ...
          if (jsExpressions.length >= 2 &&
            ...
            if (typeExpression != null) {
              ...
              myTypeEvaluateManager.setBaseType(namespace,
                currentlyDefinedTypeReference.getText(), (baseType != null ? baseType:jsExpressions[1]).getText()
              );
            }
          }
        ...
        } else if (("namespace".equals(calledMethodName) || "ns".equals(calledMethodName) || "defineClass".equals(calledMethodName) || "define".equals(calledMethodName)) &&
                   qualifier != null
                  ) {
          final JSExpression[] jsExpressions = node.getArgumentList().getArguments();
          for(JSExpression e:jsExpressions) {
            if (!(e instanceof JSLiteralExpression)) continue;

            String namespace = StringUtil.stripQuotesAroundValue(e.getText());
            final @NonNls String YAHOO_PREFIX = YAHOO_NAME + ".";
            if (!namespace.startsWith(YAHOO_PREFIX) && YAHOO_NAME.equals(qualifier.getText())) namespace = YAHOO_PREFIX + namespace;
        ...
        } else if ("addProperty".equals(calledMethodName) &&
                   qualifier != null
                  ) {
          final JSExpression[] jsExpressions = node.getArgumentList().getArguments();
        ...
        } else if (( "__defineGetter__".equals(calledMethodName) ||
                     "__defineSetter__".equals(calledMethodName)
                   ) &&
                   qualifier != null
                  ) {
          final JSExpression[] jsExpressions = node.getArgumentList().getArguments();
    ...
    @Override public void visitJSAssignmentExpression(final JSAssignmentExpression node) {
      ...
      if (_lOperand instanceof JSReferenceExpression) {
        ...
        if (currentlyDefinedTypeInstance != null &&
          ...
          suggestedNamespace = qualifierParent instanceof JSReferenceExpression ?
                               findNamespace((JSElement)qualifierParent, myThisNamespace):
                               getNestedNsWithName(lqualifier.getText(), myThisNamespace);
        }
    ...
    private void handleFunction(final JSReferenceExpression lOperand, final JSExpression rOperand,
                                final JSExpression lqualifier,
    ...
    private static boolean isTheSameExpr(final JSExpression lqualifier, JSElement currentlyDefinedTypeInstance) {
      if (lqualifier == currentlyDefinedTypeInstance) return true;

      if (currentlyDefinedTypeInstance instanceof JSVariable &&
          lqualifier instanceof JSReferenceExpression
         ) {
        return ((JSReferenceExpression)lqualifier).getReferencedName().equals(((JSVariable)currentlyDefinedTypeInstance).getName());
      }
      if (lqualifier instanceof JSReferenceExpression &&
          currentlyDefinedTypeInstance instanceof JSReferenceExpression
         ) {
    ...
    @Override public void visitJSNamespaceDeclaration(final JSNamespaceDeclaration namespaceDeclaration) {
    ...
    @Override public void visitJSPackageStatement(final JSPackageStatement packageStatement) {
    ...
    @Override public void visitJSClass(final JSClass clazz) {
        ...
        if (extendsList != null) {
          ...
          if (referenceExpressions != null) {
            for(JSReferenceExpression expr: referenceExpressions) {
              ...
              myTypeEvaluateManager.setBaseType(
                  myThisNamespace,
                  myThisNamespace.getQualifiedName(myIndex),
                  expr.getText()
              );
            }
          }
    ...
    private void processFunctionBody(final JSNamespace namespace, final JSFunction node) {
      ...
      if (node instanceof JSFunctionExpression) {
        if(nodeParent instanceof JSProperty) {
          ...
          if (qualifierString != null) myThisNamespace = getNestedNsWithName(qualifierString, myFileNamespace);
        }
      } else {
    ...
    public boolean onCommentLine(@NotNull String line) { return true; }
      ...
      if (type == MetaDocType.DEPRECATED) {
        ...
        if (myClassName != null) {
          if (remainingLineContent != null) {
            myTypeEvaluateManager.setBaseType(
                getNestedNsWithName(myClassName, myFileNamespace),
                myClassName,
    ...
    public JSAttributeList.AccessType getAccessType() {
      if (myAccessType == null) myAccessType = JSAttributeList.AccessType.PACKAGE_LOCAL;
      return myAccessType;
    }
  }
  ...
  private static JSReferenceExpression evaluateInitializedPrototype(final JSExpression initializer) {
    ...
    if (initializer instanceof JSReferenceExpression &&
        PROTOTYPE_FIELD_NAME.equals(((JSReferenceExpression)initializer).getReferencedName())
       ) {
      final JSExpression qualifier = ((JSReferenceExpression)initializer).getQualifier();
      if (qualifier instanceof JSReferenceExpression) initializedPrototype = (JSReferenceExpression)qualifier;
  ...
  private static JSReferenceExpression findReferenceExpressionUsedForClassExtending(JSReferenceExpression lOperand, @Nullable Set<String> visited) {
    ...
    if (jsElement instanceof JSVariable) {
      ...
      if (jsElement instanceof JSDefinitionExpression &&
          parentJsElement instanceof JSAssignmentExpression
        ...
        if (rOperand instanceof JSCallExpression &&
            !(rOperand instanceof JSNewExpression)) {
    ...
    if (qualifier == null) {
      ...
      if (ref instanceof JSVariable && !(ref instanceof JSParameter)) {
        ...
        if (initializer instanceof JSReferenceExpression) {
          ...
          if (visited == null) visited = new THashSet<String>();
  ...
  private static JSReferenceExpression replaceExpression(final JSReferenceExpression expression,
                                                         final JSReferenceExpression what,
                                                         final JSReferenceExpression by) {
    if (expression == what) return by;
    ...
    final JSReferenceExpression expressionToReplace =
      PsiTreeUtil.getParentOfType(copyOfExpr.findElementAt(offsetOfExprInExpression), JSReferenceExpression.class);
  ...
  public static JSElement findQualifyingExpressionFromArgumentList(final JSArgumentList parent) {
    ...
    if (grandParent instanceof JSExpressionStatement &&
        firstParent instanceof JSCallExpression
       ) {
      ...
      if ("each".equals(methodName) ||
        ...
        if (expression != null && J_QUERY_VAR_NAME.equals(expression.getText())) return expression;
  ...
  public static JSNamespace getNestedNsWithName(String name, JSNamespace contextNs, JavaScriptIndex myIndex) {
    if (!PROTOTYPE_FIELD_NAME.equals(name)) {
      ...
      if (i != name.length()) contextNs = contextNs.getChildNamespace( myIndex.getIndexOf(name.substring(lastI, name.length())) );
    }
    return contextNs;
  }

  public static @Nullable JSNamespace findNestedNsWithName(String name, JSNamespace contextNs, JavaScriptIndex myIndex) {
    if (!PROTOTYPE_FIELD_NAME.equals(name)) {
      int lastI = 0;
      int i = name.indexOf('.');
      ...
      while(i != -1) {
        ...
        if (contextNs == null) return null;
        lastI = i + 1;
        i = name.indexOf('.', lastI);
      }

      if (i != name.length()) contextNs = contextNs.findChildNamespace( myIndex.getIndexOf(name.substring(lastI, name.length())) );
    }
    return contextNs;
  }
}
