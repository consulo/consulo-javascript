...
public class JSPackage {
  ...
  synchronized void removeSubPackage(int nameId) {
    if (myChildPackages != null) myChildPackages.remove(nameId);
  }

  synchronized @NotNull JSPackage findSubPackage(int nameId) {
    if (myChildPackages == null) myChildPackages = new TIntObjectHashMap<JSPackage>(5);
  ...
  synchronized public void addInstance(@NotNull JSNamespace namespace) {
    ...
    if (myInstances != null) newInstances.addAll(myInstances);
  ...
  synchronized public void removeInstance(@NotNull JSNamespace namespace) {
    if (myInstances != null) {
      ...
      myInstances = newInstances;
      if (myInstances.size() == 0 && !myRootPackage) myParent.removeSubPackage(myNameId);
    }
  }
  ...
  public synchronized boolean processDeclarations(final JavaScriptSymbolProcessor myProcessor) {
    if (myInstances != null) {
      for(JSNamespace ns:myInstances) {
        if (!ns.processDeclarations(myProcessor)) return false;
      }
    }
  ...
  public synchronized @Nullable JSPackage findPackageWithNameId(final int packageIndex) {
    if (myChildPackages == null) return null;
