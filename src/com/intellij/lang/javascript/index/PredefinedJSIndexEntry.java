...
public class PredefinedJSIndexEntry extends JSIndexEntry
{
	...
	protected PsiFile buildPsiFileFromFile()
	{
		...
		final String jsFileName = isGeneratedCode ? JavaScriptIndex.PREDEFINES_PREFIX +
	...
	public static
	@Nullable
	PsiFile createReadOnlyFileFromText(final @Nullable String text, @NotNull Project project, @NotNull final String jsFileName)
	{
		if(text == null)
		{
			return null;
		...
		PsiFile file = PsiFileFactory.getInstance(project).createFileFromText(jsFileName, JavaScriptSupportLoader.JAVASCRIPT, text,
				LocalTimeCounter.currentTime(), false, false);
		file.putUserData(JavaScriptIndex.READONLY_JS_FILE_KEY, "");
		final VirtualFile vfile = file.getVirtualFile();
		if(vfile != null)
		{
			((LightVirtualFile) vfile).setWritable(false);
		}
				...
				try
				{
					...
					for(String propertyName : propertyNames)
					{
						...
						while(tokenizer.hasMoreTokens())
						{
							...
							if(result.length() != 0)
							{
								token = StringUtil.capitalize(token);
							}
							result.append(token);
						}
