...
public class JSResolveUtil {
  ...
  @NonNls public static final String PROTOTYPE_FIELD_NAME = "prototype";
  ...
  @NonNls private static final String ARRAY_TYPE_NAME = "Array";
  @NonNls private static final String ARGUMENTS_TYPE_NAME = "Arguments";
  ...
  public static <T extends JSNamedElement & JSAttributeListOwner> SearchScope findUseScope(final T jsVariableBase) {
    ...
    if (element != null) {
      if (element instanceof JSFunction) {
        ...
        if (elt instanceof PsiComment) return new LocalSearchScope(new PsiElement[] {elt, element});
      }
  ...
  public static final boolean isAssignableType(@NonNls final String expectedType, @NonNls String type, PsiElement context) {
    ...
    if (expectedType == null || expectedType.equals("*") || expectedType.equals(OBJECT_CLASS_NAME) || expectedType.equals("Class")) return true;
    ...
    if ("Number".equals(expectedType) && ("int".equals(type) || "uint".equals(type) || (("Boolean".equals(type) || "String".equals(type)) && nonecma))) return true;
    ...
    if ("uint".equals(expectedType) && ("int".equals(type) || "Number".equals(type))) return true;
    ...
    if (ARGUMENTS_TYPE_NAME.equals(type) && nonecma) return true; // TODO: hack for indirect call Array.slice(arguments, 0)

    if (ARRAY_TYPE_NAME.equals(expectedType) && JSTypeEvaluateManager.isArrayType(type)) {
      return true;
    }
    if ("String".equals(expectedType) && (("Number".equals(type) || "int".equals(type) || "Boolean".equals(type) || "RegExp".equals(type)) && nonecma)) return true;

    if ("*".equals(type) || (OBJECT_CLASS_NAME.equals(type) && nonecma)) return true; // Dynamic cast
    ...
    boolean result = typeClass.processDeclarations(new ResolveProcessor(null) {
      {setToProcessHierarchy(true); setTypeContext(true); setToProcessMembers(false); setLocalResolve(true);}
  ...
  private static boolean isImplicitCastPossible(JSClass typeClass, String expectedType) {
    ...
    if (expectedType.equals("flash.events.IEventDispatcher")) {
      ...
      if (attributeList != null) return attributeList.getAttributesByName("Bindable").length != 0;
    }
    return false;
  }
  ...
  public static PsiElement getTopReferenceExpression(final @NotNull PsiElement parent) {
    ...
    for(PsiElement currentParent = parent.getParent();
        currentParent instanceof JSReferenceExpression;
  ...
  private static PsiElement findParentClass(final JSFile file) {
    ...
    if (forcedContext != null && !forcedContext.isValid()) forcedContext = null;
  ...
  public static @Nullable JSClass findClassOfQualifier(final JSExpression qualifier, final PsiFile containingFile) {
    ...
    if (qName instanceof JSClass) return (JSClass)qName;
    return null;
  }
  ...
  public static Collection<JSQualifiedNamedElement> findElementsByName(String name, Project project, GlobalSearchScope scope) {
    ...
    files.addAll(FileBasedIndex.getInstance().getContainingFiles(FilenameIndex.NAME, name + JavaScriptSupportLoader.MXML_FILE_EXTENSION_DOT,
			scope));
    files.addAll(FileBasedIndex.getInstance().getContainingFiles(FilenameIndex.NAME, name + JavaScriptSupportLoader.MXML_FILE_EXTENSION2_DOT,
			scope));
  ...
  public static String getTypeFromSetAccessor(final JSNamedElement jsNamedElement) {
    if (!(jsNamedElement instanceof JSFunction) || !((JSFunction)jsNamedElement).isSetProperty()) return null;
    final JSParameterList jsParameterList = ((JSFunction)jsNamedElement).getParameterList();
    if (jsParameterList == null) return null;
    final JSParameter[] jsParameters = jsParameterList.getParameters();
    if (jsParameters == null || jsParameters.length != 1) return null;
  ...
  public static boolean processTopPackages(final ResolveProcessor processor, final ResolveState state, final Project project,
    ...
    return JSPackageIndex.processElementsInScope(
      "", processor.getName(),
  ...
  public static boolean processTopLevelClasses(PsiScopeProcessor processor, ResolveState state, Project project,
                                                GlobalSearchScope scope, boolean acceptOnlyClasses, boolean acceptQualifiedElements) {
    boolean result = true;
    ...
    if (resolvedName == null) {
      for(String s : StubIndex.getInstance().getAllKeys(JSNameIndex.KEY, project)) {
        for(JSQualifiedNamedElement e:StubIndex.getInstance().get(JSNameIndex.KEY, s, project, scope)) {
          if (ARGUMENTS_TYPE_NAME.equals(e.getName())) continue;
          if (acceptOnlyClasses && !(e instanceof JSClass)) continue;
      ...
      for(JSQualifiedNamedElement e: StubIndex.getInstance().get(JSQualifiedElementIndex.KEY, resolvedName.hashCode(), project, scope)) {
        if (!e.getName().equals(resolvedName)) continue;
        if (acceptOnlyClasses && !(e instanceof JSClass)) continue;
        result = processor.execute(e, state);
        if (!result) break;
      }
  ...
  static boolean walkOverStructure(@NotNull PsiElement context, Processor<PsiNamedElement> processor) {
    ...
    while(parent != null) {
      ...
      if (parent instanceof JSPackageStatement) break;
  ...
  private static final UserDataCache<ParameterizedCachedValue<Set<String>, JSElement>, JSElement, Object> ourCachedOpenedNsesCache =
    new UserDataCache<ParameterizedCachedValue<Set<String>, JSElement>, JSElement, Object>() {
    @Override
          ...
          public CachedValueProvider.Result<Set<String>> compute(JSElement context) {
            class MyProcessor extends ResolveProcessor implements Processor<PsiNamedElement> {
              ...
              public boolean execute(PsiElement element, ResolveState state) {
                ...
                if (openedNses == null) openedNses = new THashSet<String>();
  ...
  public static boolean processGlobalThings(PsiScopeProcessor processor, ResolveState state, PsiElement place, PsiElement context) {
    ...
    if (result) {
      boolean acceptOnlyClasses = place instanceof JSReferenceExpression
                                  ? isExprInTypeContext((JSReferenceExpression)place):false;
  ...
  public static @Nullable JSParameter findParameterForUsedArgument(@NotNull JSExpression mainOccurence, @NotNull JSArgumentList parent) {
    int paramIndex = 0;

      for(JSExpression expr:parent.getArguments()) {
        if (expr == mainOccurence) break;
        paramIndex++;
      }
  ...
  public static boolean processAllGlobals(final PsiScopeProcessor processor, final XmlFile xmlFile, final PsiElement place) {
    ...
    if (!clazz.processDeclarations(processor, ResolveState.initial(), clazz, place)) return false;
    return true;
  }
  ...
  public static boolean isReferenceTo(final PsiPolyVariantReference reference,
    ...
    else if (_element instanceof PsiFile) elementName = ((PsiFile)_element).getVirtualFile().getNameWithoutExtension();
    else if (_element instanceof PsiDirectoryContainer) elementName = ((PsiNamedElement)_element).getName();
    else return false;
    ...
    if (Comparing.equal(referencedName, elementName, true)) {
      ...
      for (ResolveResult r:resolveResults) {
        ...
        if (resolvedElement instanceof JSNamedElementProxy) resolvedElement = ((JSNamedElementProxy)resolvedElement).getElement();
        ...
        if (resolvedElement.isEquivalentTo(element) ||
          ...
          if (reference instanceof JSReferenceExpression &&
              ( (((JSReferenceExpression)reference).getParent() == resolvedElement && resolvedElement instanceof JSDefinitionExpression) ||
                resolvedElement instanceof JSFunctionExpression &&
                ((JSFunctionExpression)resolvedElement).getFunction().findNameIdentifier().getTreeParent().getPsi() == reference
              )
            ) {
        ...
        if (resolvedElement instanceof JSFunctionExpression) {
          ...
          if (fun.isConstructor()) {
            if ((element instanceof JSClass && resolvedElement.getParent() == element) ||
        ...
        if ((resolvedElement instanceof JSClass ||
                   ...
                   (( element instanceof XmlFile &&
                     resolvedElement.getParent().getContainingFile() == element
                    ) ||
                    (element instanceof JSFile &&
                     reference.getElement().getParent() != resolvedElement &&
  ...
  private static boolean fieldIsImplicitAccessorMethod(JSFunction fun, JSVariable var) {
    if (!fun.isGetProperty() && !fun.isSetProperty()) return false;
    PsiElement funParent = findParent(fun);
    if (!(funParent instanceof JSClass) || !((JSClass)funParent).isInterface()) return false;
    ...
    if (!(parent instanceof JSClass)) return false;

    for(JSClass c:((JSClass)parent).getImplementedInterfaces()) {
      if (c == funParent) return true;
    }
  ...
  private static boolean isPublicEntityReferenceToJSFile(PsiElement resolvedElement, PsiElement element) {
    ...
    return parent instanceof JSPackageStatement &&
  ...
  public static boolean checkClassHasParentOfAnotherOne(final JSClass aClass, final JSClass parent, @Nullable Set<JSClass> visited) {
    if (visited != null && visited.contains(aClass)) return false;

    for(JSClass superClazz: aClass.getSuperClasses()) {
      if (superClazz == parent) return true;
      if (visited == null) visited = new THashSet<JSClass>();
      visited.add(aClass);
  ...
  public static @Nullable JSExpression getRealRefExprQualifier(final JSReferenceExpression expr) {
    ...
    if (isExprInTypeContext(expr)) {
      ...
      if (resolved == null) return expr.getQualifier();
  ...
  private static PsiElement findClassByQName(final String link, final JavaScriptIndex index, final GlobalSearchScope searchScope) {
    synchronized (index) {
      ...
      for(JSQualifiedNamedElement clazz:candidates) {
        if (link.equals(clazz.getQualifiedName())) {
          if ("Object".equals(link) &&
              !JavaScriptIndex.ECMASCRIPT_JS2.equals(clazz.getContainingFile().getVirtualFile().getName()) // object from swf do not contain necessary members!
            ) {
            continue;
          }
          result[0] = clazz;
      ...
      if (psiElement != null) index.rememberTopLevelClassElement(link, searchScope, psiElement);
      return psiElement;
    }
  }
  ...
  public static boolean isPredefinedFile(PsiFile file) {
    if (file instanceof JSFile) return ((JSFile)file).isPredefined();
    return false;
  }
  ...
  public static JSPackage findPackageByText(final String str,final JavaScriptIndex index) {
    ...
    if (element instanceof MyPackageWrapper) return ((MyPackageWrapper)element).myPackage;
    return null;
  }
  ...
  private static PsiElement doFindPackage(final String qName, final JavaScriptIndex index, @Nullable Module module) {
    ...
    synchronized (index) {
      ...
      if (myWrapper != null) index.rememberPackageElement(qName, myWrapper);
      return myWrapper;
    }
  }
  ...
  public static JSExpression findClassIdentifier(JSExpression _qualifier) {
    ...
    if (qualifier instanceof JSReferenceExpression &&
        PROTOTYPE_FIELD_NAME.equals(((JSReferenceExpression)qualifier).getReferencedName())
      ) {
  ...
  public static boolean iterateType(final JSFunction node, final PsiElement jsClass, final String typeName, final OverrideHandler handler) {
    ...
    if (jsClass instanceof JSClass) {
      ...
      if (attributeList != null) namespace = attributeList.getNamespace();
      return processOverrides(jsClass, handler, node.getName(), namespace, node);
    }

    return JSTypeEvaluateManager.getInstance(project).iterateTypeHierarchy(typeName,
        ...
        public boolean process(final JSNamespace jsNamespace) {
          if (visited.contains(jsNamespace)) return true;
  ...
  public static boolean processOverrides(final PsiElement jsClass, final OverrideHandler handler, String name, final Object namespace, final PsiElement context) {
    ...
    final ResolveProcessor resolveProcessor = new ResolveProcessor(name, context) {{setToProcessHierarchy(true); }
    ...
    for(JSClass superClazz:clazz.getSuperClasses()) {
      ...
      if (!b) {
        ...
        if (parent instanceof JSClass && !handler.process(resolveProcessor, superClazz, ((JSClass)parent).getQualifiedName() )) return false;
      }
    }
  ...
  private static boolean doProcessMetaAttributesForClass(final PsiElement jsClass, final MetaDataProcessor processor, PsiElement lastParent, boolean forward) {
    if (jsClass instanceof JSClass) {
      ...
      while(ind >=0 && elements[ind] != jsClass) --ind;
      -- ind;
        ...
        while(ind >= 0) {
          ...
          if (current instanceof JSIncludeDirective) {
            if (!processIncludeDirective(processor, jsClass, (JSIncludeDirective)current, false)) return false;
            --ind;
          } else {
            break;
          }
    ...
    for(int i = forward ?  0 : elements.length - 1; i < elements.length && i >= 0; i += (forward ? 1:-1)) {
      ...
      if (el instanceof JSIncludeDirective) {
        if (!processIncludeDirective(processor, lastParent, (JSIncludeDirective)el, forward)) return false;
        ...
        if (!processor.handleOtherElement(el, jsClass, continuePassElement)) return false;
        ...
        if (nextEl instanceof JSAttributeListOwner) {
          ...
          if (attributeList != null) {
            if (!processAttributeList(processor, nextEl, attributeList, forward)) return false;
          }
        }
  ...
  public static boolean processAttributeList(final MetaDataProcessor processor, final PsiElement el,
    ...
    for(int i = forward ?  0 : elements.length - 1; i < elements.length && i >= 0; i += (forward ? 1:-1)) {
      ...
      if (cur instanceof JSIncludeDirective) {
        if (!processIncludeDirective(processor, el, (JSIncludeDirective)cur, forward)) return false;
    ...
    if (!processor.handleOtherElement(attributeList, el, null)) return false;

    return true;
  }

  private static boolean processIncludeDirective(final MetaDataProcessor processor, final PsiElement lastParent,
                                                 final JSIncludeDirective el, boolean forward) {
    final PsiFile file = el.resolveFile();

    if (file instanceof JSFile) {
      if(!doProcessMetaAttributesForClass(file, processor, lastParent, forward)) return false;
    }
  ...
  private static boolean dispatchResolve(final PsiScopeProcessor processor, final ResolveState state, final PsiElement place,
                                        boolean result, final Object o) {
  ...
  public static class RelevantDefsUserDataCache extends UserDataCache<CachedValue<TIntObjectHashMap<Object>>, JSElement, Object> {
    ...
    private static class MyJSElementVisitor extends JSElementVisitor {
      ...
      @Override public void visitJSDefinitionExpression(final JSDefinitionExpression node) {
      ...
      public void visitJSIncludeDirective(final JSIncludeDirective includeDirective) {
        ...
        if (!(_file instanceof JSFile) || myBase.getContainingFile() == _file) return;
        final JSFile file = (JSFile)_file;
        if (visitedIncludes != null && visitedIncludes.contains(file)) return;
        if (visitedIncludes == null) visitedIncludes = new THashSet<JSFile>();
      ...
      public void visitJSVarStatement(final JSVarStatement node) {
        for(JSVariable var:node.getVariables()) {
          if (!var.isLocal()) var.accept(this);
        }
      }

      @Override public void visitJSParameter(JSParameter node) {}

      @Override public void visitJSFunctionExpression(final JSFunctionExpression node) {
        // do not go inside other funcs
      }

      @Override public void visitJSObjectLiteralExpression(final JSObjectLiteralExpression node) {
        // do not go inside other funcs
      }

      @Override public void visitJSClass(final JSClass aClass) {
        // do not go inside other funcs
      }
      ...
      public void visitJSUseNamespaceDirective(final JSUseNamespaceDirective useNamespaceDirective) {
        ...
        if (namespaceToBeUsed != null) addNamedElement(useNamespaceDirective, namespaceToBeUsed.hashCode());
      }
        ...
        while(parentContainer instanceof JSFunctionExpression) {
          ...
          if (parentContainerParent instanceof JSAssignmentExpression) {
            ...
            if (element instanceof JSVariable) {
              ...
              if (loperand != null) qualifyingExpression = ((JSDefinitionExpression)loperand).getExpression();
  ...
  public static abstract class CollectMethodsToImplementProcessor extends ResolveProcessor {
    ...
    public boolean execute(final PsiElement element, final ResolveState state) {
      final ResolveProcessor processor = new ResolveProcessor(myName, place) {
        ...
        public boolean execute(final PsiElement element, final ResolveState state) {
          if (element instanceof JSFunction) {
            if (OBJECT_CLASS_NAME.equals(((JSClass)findParent((JSNamedElement)element)).getQualifiedName()) ||
      ...
      if (myName == null && processor.getResults() != null) process(processor);
      return false;
    }
  ...
  static JSSourceElement[] getSourceElements(final PsiElement owner) {
    ...
    else if (owner instanceof JSFile) return ((JSFile)owner).getStatements();
  ...
  public static PsiElement[] getStubbedChildren(final PsiElement owner, TokenSet filter) {
    if (owner instanceof JSStubElementImpl) {
      ...
      if (stub != null) return stub.getChildrenByType(filter, PsiElement.EMPTY_ARRAY);
  ...
  private static PsiElement[] getChildrenFromTokenSet(final ASTNode astNode, TokenSet filter) {
    ...
    for(int i = 0; i < astNodes.length;++i) result[i] = astNodes[i].getPsi();
    return result;
  }

  private static final TokenSet ourStubbedFilter = TokenSet.create(
      JSElementTypes.CLASS, JSElementTypes.VAR_STATEMENT, JSElementTypes.FUNCTION_DECLARATION, JSElementTypes.ATTRIBUTE_LIST,
  ...
  private static class ImplicitJSVariableImpl extends LightElement implements JSVariable {
    ...
    public boolean isEquivalentTo(PsiElement another) {
      return another == this ||
             (another instanceof ImplicitJSVariableImpl &&
