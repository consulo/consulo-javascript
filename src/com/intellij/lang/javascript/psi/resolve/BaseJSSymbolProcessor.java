...
abstract public class BaseJSSymbolProcessor implements JavaScriptSymbolProcessor, PsiScopeProcessor {
  ...
  @NonNls public static final String ARRAY_TYPE_NAME = "Array";
  @NonNls public static final String STRING_TYPE_NAME = "String";
  @NonNls public static final String REG_EXP_TYPE_NAME = "RegExp";
  @NonNls public static final String XML_TYPE_NAME = "XML";
  @NonNls public static final String XML_LIST_TYPE_NAME = "XMLList";
  @NonNls public static final String NUMBER_TYPE_NAME = "Number";
  @NonNls public static final String FUNCTION_TYPE_NAME = "Function";
  @NonNls public static final String HTML_ELEMENT_TYPE_NAME = "HTMLElement";
  @NonNls private static final String INT_TYPE = "int";
  @NonNls private static final String BOOLEAN_TYPE_NAME = "Boolean";
  protected String myIteratedTypeName;

  protected BaseJSSymbolProcessor(final PsiFile targetFile,
    ...
    if ( ( ( contextIds == null ||
          ...
          JSResolveUtil.getRealRefExprQualifier((JSReferenceExpression)myContext) == null
         ) ||
         (
           contextIds != null && contextIds.length == 1 && contextIds[0] == myWindowIndex && notWithinWithStatement
         )
       ) {
  ...
  public static void doEvalForExpr(JSExpression rawqualifier, final PsiFile myTargetFile, TypeProcessor typeProcessor) {
    ...
    try {
      ...
      if (!contextHolderInitialized) context.removeProcessingItem(rawqualifier);
      if (contextHolderInitialized) contextHolder.set(null);
    }
  }
          ...
          if (at instanceof PsiLanguageInjectionHost) {
              ...
              for(Pair<PsiElement, TextRange> pair: injectedPsiFiles) {
                if (pair.getFirst() instanceof JSFile) {
                  originalFile = (PsiFile)pair.getFirst(); break;
                }
              }
            }
          }
  ...
  private static void doEvalForExpr(JSExpression rawqualifier, TypeProcessor typeProcessor, EvaluateContext context) {
    ...
    if (rawqualifier instanceof JSReferenceExpression) {
      ...
      if (topLevelQualifier) {
        final JSReferenceExpression expression = typeProcessor.ecma() ?
                                                 qualifier:
                                                 JSSymbolUtil.findReferenceExpressionUsedForClassExtending(qualifier);
          ...
          if (type == null) {
            if (psiElement instanceof JSVariable) {
              ...
              if (isSomeFunctionCall(rawqualifier, parameterType)) {
                ...
                if (jsVariable.hasInitializer()) {
                  ...
                  if (jsExpression instanceof JSFunctionExpression) {
                    ...
                    if (isValidType(typeString)) parameterType = typeString;
                  }
                }
              }
              ...
              if (!typeProcessor.ecma() || parameterType == null) {
                ...
                if (expression != null) {
                  ...
                  if (!hasSomeInfoAboutVar && parameterType == null) {
                    ...
                    if ((grandParent = jsVariable.getParent().getParent()) instanceof JSForInStatement &&
                        jsVariable.getParent() == ((JSForInStatement)grandParent).getDeclarationStatement()
                       ) {
                  ...
                  if (!hasSomeInfoAboutVar && parameterType == null) typeProcessor.setUnknownElement(jsVariable);
                }
              }
            } else {
              ...
              if (psiElement instanceof JSNamedElementProxy &&
                  ( psiElement.getContainingFile() == context.targetFile ||
                    psiElement.getContainingFile() instanceof XmlFile // injected
                  )
                 ) {
              ...
              if (psiElement instanceof JSClass) {
                ...
                if ((parent = psiElement.getParent()) instanceof JSForInStatement &&
                     psiElement == ((JSForInStatement)parent).getVariableExpression()
                    ) {
                  parameterType = evalComponentTypeFromArrayExpression(
                    rawqualifier,
                    typeProcessor,
                ...
                if (parameterType == null) addTypeFromDefExpr(typeProcessor, context, psiElement);
                ...
                if (psiElement instanceof JSPackageWrapper) {
                  ...
                  if (!inCall && (!function.isGetProperty() && !function.isSetProperty())) {
                    ...
                    if (wasPrototype) {
                      ...
                      if (name != null) addType(name, typeProcessor, context, null);
                    }
                  } else {
                  ...
                  if (!(rawqualifier.getParent() instanceof JSCallExpression) &&
                      psiElement instanceof JSNamedElementProxy
                     ) {
              ...
              if (!hasSomeType && psiElement != null) typeProcessor.setUnknownElement(psiElement);
            }
      ...
      if (rOperand != null) {
        if (sign == JSTokenTypes.AS_KEYWORD) {
          ...
          if (evaluatedType != null) addType(evaluatedType, typeProcessor, context, rawqualifier);
          else typeProcessor.setUnknownElement(rawqualifier);
      ...
      if (lprocessor.result instanceof JSArrayLiteralExpression && typeProcessor instanceof ResolveProcessor) {
        for(JSExpression expr:((JSArrayLiteralExpression)lprocessor.result).getExpressions()) {
          if (expr instanceof JSObjectLiteralExpression) addTypeFromObjectLiteralExpression(expr, typeProcessor);
        }
      }
      ...
      if (jsClass != null) addType(jsClass.getQualifiedName(), typeProcessor, context, jsClass);
      else typeProcessor.setUnknownElement(rawqualifier);
    } else if (rawqualifier instanceof JSSuperExpression) {
      final JSClass jsClass = JSResolveUtil.getClassOfContext(rawqualifier);
      if (jsClass != null) {
        final JSClass[] classes = jsClass.getSuperClasses();
        if (classes.length > 0) addType(classes[0].getQualifiedName(), typeProcessor, context, classes[0]);
      }
    }
  ...
  private static boolean isSomeFunctionCall(JSExpression rawqualifier, String parameterType) {
    return FUNCTION_TYPE_NAME.equals(parameterType) &&
        rawqualifier.getParent() instanceof JSCallExpression;
  }
  ...
  private static void addQNameFromElementAsType(TypeProcessor typeProcessor, EvaluateContext context, PsiElement psiElement, boolean passSource) {
    ...
    if (psiElement instanceof JSQualifiedNamedElement) {
      ...
      if (qName != null) name = qName;
    }
    addType(name, typeProcessor, context, passSource ? psiElement:null);
  }

  private static String evalComponentTypeFromArrayExpression(JSExpression rawqualifier,
                                                           TypeProcessor typeProcessor,
                                                           EvaluateContext context,
                                                           JSExpression collectionExpression) {
    if (collectionExpression != null) {
      if (context.isAlreadyProcessingItem(rawqualifier)) return null;
      context.addProcessingItem(rawqualifier);
      final SimpleTypeProcessor lprocessor = new SimpleTypeProcessor(typeProcessor.ecma());
  ...
  private static String addComponentTypeFromProcessor(JSExpression rawqualifier,
                                                    TypeProcessor typeProcessor,
                                                    EvaluateContext context,
  ...
  private static void tryAddBindowsType(final JSReferenceExpression qualifier, final TypeProcessor typeProcessor,
                                        final EvaluateContext context) {
    final PsiElement element = qualifier.getParent();
    if (!(element instanceof JSCallExpression)) return;
    ...
    if (expressions.length == 0 || !(expressions[0] instanceof JSLiteralExpression)) return;
    ...
    if (contextElement != null) {
      ...
      if (isBindowsXml(containingFile)) {
        ...
        index.getEntryForFile(containingFile).processSymbols(
          new DefaultSymbolProcessor() {
  ...
  private static void addTypeFromObjectLiteralExpression(final JSExpression rawqualifier, final TypeProcessor typeProcessor) {
    ...
    final boolean b = properties.length > 0 ?
                      rawqualifier.processDeclarations((ResolveProcessor)typeProcessor, ResolveState.initial(), properties[0], properties[0]):
                      true;
    if (b) typeProcessor.setUnknownElement(rawqualifier);
  }
  ...
  private static void addTypeFromDefExpr(final TypeProcessor typeProcessor, final EvaluateContext context, final PsiElement psiElement) {
    ...
    if (!context.visitedTypes.contains(type)) {
      ...
      if (parentElement instanceof JSAssignmentExpression) {
        ...
        while(expr instanceof JSAssignmentExpression) expr = ((JSAssignmentExpression)expr).getROperand();
        if (expr != null) {
  ...
  private static void getTypeFromConstant(final JSExpression rawqualifier, TypeProcessor typeProcessor, EvaluateContext context) {
    ...
    String type = constantType == JSTokenTypes.STRING_LITERAL || constantType == JSTokenTypes.SINGLE_QUOTE_STRING_LITERAL
                        ? STRING_TYPE_NAME
                        : constantType == JSTokenTypes.NUMERIC_LITERAL
                          ? NUMBER_TYPE_NAME
                          : constantType == JSTokenTypes.REGEXP_LITERAL ? REG_EXP_TYPE_NAME
                            : constantType == JSTokenTypes.XML_START_TAG_START ? XML_TYPE_NAME
                            : constantType == JSTokenTypes.XML_START_TAG_LIST ? XML_LIST_TYPE_NAME:
                              constantType == JSTokenTypes.TRUE_KEYWORD  || constantType == JSTokenTypes.FALSE_KEYWORD ? BOOLEAN_TYPE_NAME:
                              null;
  ...
  protected void addPackageScope(final List<int[]> possibleNameIds, final JSClass jsClass, final PsiElement expression) {
    ...
    if (packageQualifier != null) {
      ...
      if (index > 0) buildIndexListFromQNameAndCorrectQName(qName.substring(0, index), jsClass, possibleNameIds);
    } else if (jsClass == null) {
      String s = JSResolveUtil.findPackageForMxml(expression);
      if (isValidType(s)) buildIndexListFromQNameAndCorrectQName(s, expression, possibleNameIds);
    }
  }
  ...
  public static String addIndexListFromQName(String type, final PsiElement source, final TIntArrayList list, JavaScriptIndex myIndex) {
    ...
    if (i != type.length()) {
      ...
      if (source == null) s = StringUtil.capitalize(s); // when type name goes from comments
      if (lastI == 0 && !s.equals(type)) type = s;
  ...
  private static void addType(String type, TypeProcessor typeProcessor, EvaluateContext context, final PsiElement source) {
    ...
    if (spacePos != - 1) type = type.substring(0, spacePos);
    ...
    if (!typeProcessor.ecma()) type = context.typeEvaluateManager.getInstanceNameByType(type);
  ...
  protected void doIterateHierarchy(final String typeName, final HierarchyProcessor processor) {
        ...
        superClazz.processDeclarations(new ResolveProcessor(null) {
          ...
          public boolean execute(final PsiElement element, final ResolveState state) {
            if (element instanceof JSClass) {
              if (!processor.processClass((JSClass)element)) return false;
            }
            return true;
          }
    ...
    JSTypeEvaluateManager.getInstance(myContext.getProject()).iterateTypeHierarchy(typeName,
  ...
  public static class EvaluateContext {
    ...
    public void addProcessingItem(final JSExpression rawqualifier) {
      if (processingItems == null) processingItems = new THashSet<JSExpression>();
