...
public class WalkUpResolveProcessor extends BaseJSSymbolProcessor {
    ...
    if (context instanceof JSReferenceExpression) {
      ...
      if (qualifier instanceof JSThisExpression ||
        ...
        if (qualifier instanceof JSThisExpression) {
          ...
          if (clazz instanceof JSClass) {
            ...
            if ((attrList = ((JSClass)clazz).getAttributeList()) != null) {
              ...
              if (clazzIsDynamic) haveNotEncounteredDynamics = false;
            }
        ...
        if (qualifier == null) {
          ...
          if (expression != null) {
            possibleNameIds
              .add(JSResolveUtil.buildNameIdsForQualifier(JSResolveUtil.getRealRefExprQualifierFromResult(refExpr, expression), myIndex));
          }
          else {
      ...
      else if (myContextIds == null && contextIds != null) myContextIds = new int[][]{contextIds};
    }
  ...
  private void iterateContextIds(final int[] contextIds, final List<int[]> possibleNameIds, final boolean allowObject) {
    doIterateTypeHierarchy(contextIds, new HierarchyProcessor() {
      public boolean processNamespace(final JSNamespace ns) {
        if ("Object".equals(ns.getQualifiedName(myIndex)) && !allowObject) return true;
        possibleNameIds.add(ns.getIndices());
        return true;
      }
    ...
    if (partialMatch) {
      if (myInNewExpression) {
        ...
        if (type == JSNamedElementProxy.NamedItemType.MemberFunction && proxy.hasProperty(JSNamedElementProxy.Property.Constructor)) {
          ...
          final boolean canResolveToElement = !qualifiedContext == inglobalNs;
          return proxy.hasProperty(JSNamedElementProxy.Property.HasConstructor) || !canResolveToElement
                 ? MatchType.NOMATCH
                 : MatchType.COMPLETE;
        }
      }
      ...
      if (myContextIds != null) {
        ...
        for (int currentContextIndex = 0; currentContextIndex < maxContextScanCount; ++currentContextIndex) {
          ...
          for (i = contextIds.length - 1; i >= 0; --i) {
            ...
            if (namespace.getNameId() != contextIds[i]) break;
    ...
    if (matchType == MatchType.PARTIAL) {
      ...
      if (myDefinitelyGlobalReference &&
          ((element instanceof JSNamedElementProxy &&
            ((type = ((JSNamedElementProxy)element).getType()) == JSNamedElementProxy.NamedItemType.FunctionProperty ||
             ...
             (type == JSNamedElementProxy.NamedItemType.Definition && !isGlobalNS(namespace)))) ||
      ...
      if (myDefinitelyNonglobalReference &&
          (element instanceof JSNamedElementProxy &&
           ((type = ((JSNamedElementProxy)element).getType()) == JSNamedElementProxy.NamedItemType.Variable ||
            type == JSNamedElementProxy.NamedItemType.Function)) ||
          (element instanceof JSVariable || (element instanceof JSFunction && !(element instanceof JSFunctionExpression)))) {
  ...
  protected boolean shouldProcessVariable(final int nameId, JSNamedElement var) {
    return (myNameId == nameId &&
            (!myDefinitelyNonglobalReference ||
             (var instanceof JSNamedElementProxy &&
              ((JSNamedElementProxy)var).getType() == JSNamedElementProxy.NamedItemType.MemberVariable)));
  }
  ...
  public ResolveResult[] getResults() {
    ...
    if (myPartialMatchResults != null && addPartialResults) resultCount += myPartialMatchResults.size();
    ...
    if (myPartialMatchResults != null && addPartialResults) {
      ...
      for (int i = 0; i < myPartialMatchResults.size(); ++i) {
        ...
        if (myCompleteMatchResults == null &&
            (myCanHaveValidPartialMatches ||
             (resolveResult.getElement() instanceof JSNamedElementProxy &&
              (//(type = ((JSNamedElementProxy)resolveResult.getElement()).getType()) == JSNamedElementProxy.NamedItemType.Namespace ||
  ...
  public boolean execute(final PsiElement element, final ResolveState state) {
    if ((element instanceof JSNamedElement && myIndex.getIndexOf(((JSNamedElement)element).getName()) == myNameId) ||
        element == myContext) {
      addCompleteResult(element);
    }
    return true;
