...
public class ResolveProcessor extends UserDataHolderBase implements PsiScopeProcessor
{
	...
	public boolean execute(PsiElement element, ResolveState state)
	{
		...
		if(place != null && element instanceof JSFunction && !(element instanceof JSFunctionExpression))
		{
			...
			{
				PsiElement clazz;
				if(function.isGetProperty() && (myName != null || !(placeParent instanceof JSExpressionStatement) || !((clazz = function.getParent()) instanceof
						JSClass) ||
	...
	public void handleEvent(Event event, Object associated)
	{
		if(event == Event.SET_DECLARATION_HOLDER)
		{
			if(associated instanceof JSClass)
			{
				...
				if(processStatics)
				{
					if(myTypeName != null && !myTypeName.equals(jsClass.getQualifiedName()) || myTypeName == null && myClassScopeTypeName != null &&
	...
	public ResolveResult[] getResultsAsResolveResults()
	{
		...
		for(int i = 0; i < results.length; ++i)
		{
			...
			results[i] = new JSResolveUtil.MyResolveResult(element, myImportsUsed != null ? myImportsUsed.get(i) : null,
