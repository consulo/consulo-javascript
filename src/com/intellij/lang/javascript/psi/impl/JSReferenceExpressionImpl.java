...
public class JSReferenceExpressionImpl extends JSExpressionImpl implements JSReferenceExpression, BindablePsiReference {
  ...
  public PsiElement bindToElement(@NotNull PsiElement element) throws IncorrectOperationException {
    ...
    if (parent instanceof JSClass ||
      ...
      if (node != null && node.getPsi() == this) {
        if (parent == element || element instanceof PsiFile) return this; // JSNamedElement.setName will care of things
      }
    }
  ...
  private void doProcessLocalDeclarations(final JSExpression qualifier, final ResolveProcessor processor, boolean ecma,
                                             boolean completion) {
    ...
    if (elToProcess == null && whereTypeCanBe) {
      ...
      if (elToProcess != null) {
        ...
        if (elToProcess == null) elToProcess = this;
      }
    }
  ...
  private static @Nullable JSClass findEnclosingClass(PsiElement elt) {
    ...
    if (isLocalResolveQualifier(qualifier)) {
      ...
      if (processor.resolved == MyTypeProcessor.TypeResolveState.Resolved ||
        ...
        if (JSResolveUtil.isSelfReference(getParent(), this)) {
          ...
          if (originalParent instanceof JSQualifiedNamedElement) qualifiedNameToSkip = ((JSQualifiedNamedElement)originalParent).getQualifiedName();
        }
  ...
  private ResolveResult[] doResolve(PsiFile containingFile) {
    ...
    if("*".equals(referencedName) && currentParent instanceof JSImportStatement && qualifier instanceof JSReferenceExpression) { // TODO: move to some processor
    ...
    ResolveResult[] results =
      doOldResolve(containingFile, referencedName, parent, qualifier, ecma, localResolve, parentIsDefinition, index, localProcessor);
    ...
    if (qualifier instanceof JSReferenceExpression) {
      ...
      if (node.getElementType() == JSTokenTypes.AT) {
        ...
        if (parent instanceof JSBinaryExpression) {
          ...
          if (element instanceof JSCallExpression) parent = ((JSCallExpression)element).getMethodExpression();
        }
        if (parent instanceof JSExpression) return (JSExpression)parent;
        return null;
      }
    }
    return qualifier;
...

  {
    ...
    public void process(String type,
                        @NotNull final BaseJSSymbolProcessor.EvaluateContext evaluateContext, PsiElement source) {
      ...
      final PsiElement clazz = source != null && (source instanceof JSClass || source instanceof XmlFile) ?
                               source:JSClassImpl.findClassFromNamespace(type, place);
      ...
      if (clazz instanceof JSClass) {
        ...
        final boolean statics =
            myEcma && JSPsiImplUtils.isTheSameClass(typeSource, jsClass) && !(((JSReferenceExpression)place).getQualifier() instanceof JSCallExpression);
    ...
    public boolean execute(PsiElement element, ResolveState state) {
      ...
      if (myEcma && getResult() != null) resolved = MyTypeProcessor.TypeResolveState.Resolved;
      return b;
    }
