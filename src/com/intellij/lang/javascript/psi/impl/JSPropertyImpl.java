...
public class JSPropertyImpl extends JSElementImpl implements JSProperty {
  private static TokenSet IDENTIFIER_TOKENS_SET = TokenSet.orSet(
    JSTokenTypes.IDENTIFIER_TOKENS_SET,
    TokenSet.create(JSTokenTypes.STRING_LITERAL, JSTokenTypes.NUMERIC_LITERAL)
  );

  private static TokenSet FUNCTION_TOKEN_SET = TokenSet.create(
  ...
  static boolean isGetIdentifier(final ASTNode node) {
    ...
    return type == JSTokenTypes.GET_KEYWORD ||
      type == JSTokenTypes.IDENTIFIER && "get".equals(node.getText());
  }
  ...
  static boolean isSetIdentifier(final ASTNode node) {
    ...
    return type == JSTokenTypes.SET_KEYWORD ||
      type == JSTokenTypes.IDENTIFIER && "set".equals(node.getText());
  }
