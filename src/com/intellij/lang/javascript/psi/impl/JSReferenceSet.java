...
public class JSReferenceSet {
  ...
  private class MyPsiReference implements PsiPolyVariantReference, EmptyResolveMessageProvider {
    ...
    public PsiElement bindToElement(@NotNull PsiElement element) throws IncorrectOperationException {
      ...
      if (qName != null) {
        handleContentChange(getElement(), new TextRange(myReferences[0].getRangeInElement().getStartOffset(), getRangeInElement().getEndOffset()), qName);
      }
      return null;
    }
    ...
    public Object[] getVariants() {
      ...
      if (isNewResolveAndCompletion(containingFile)) {
        ...
        if (!(element instanceof JSLiteralExpression)) return processor.getResultsAsObjects();
      }
    ...
    private Object[] getOldVariants(PsiFile containingFile, ResolveProcessor localProcessor) {
      ...
      if (localProcessor != null) processor.addLocalResults(localProcessor.getResults());  // TODO: remove this stuff as we create new js index
    ...
    private @Nullable TIntArrayList fillContextIds(final JavaScriptIndex index) {
      ...
      if (contextIds == null && myOffset > 0) {
        ...
        if (psiElement instanceof XmlToken) {
          final BaseJSSymbolProcessor.TagContextBuilder builder = new BaseJSSymbolProcessor.TagContextBuilder(psiElement, BaseJSSymbolProcessor.HTML_ELEMENT_TYPE_NAME);
    ...
    private ResolveProcessor doProcess(PsiFile psiFile, String text) {
      ResolveProcessor processor = new ResolveProcessor(text) {
        ...
        public boolean execute(PsiElement element, ResolveState state) {
          if (onlyPackages && !(element instanceof JSPackageWrapper)) return true;
          if (onlyFqns && !(element instanceof JSPackageWrapper) && !(element instanceof JSClass)) return true;
      ...
      if (i == 0) {
        ...
        if (myOffset > 0) {
          if (elt instanceof JSClass && !(element instanceof JSLiteralExpression)) {
            ...
            if(!elt.processDeclarations(processor, ResolveState.initial(), elt, elt)) return processor;
          }
      ...
      if (psiFile instanceof XmlFile && !JavaScriptSupportLoader.isFlexMxmFile(psiFile)) {
        ...
        JSResolveUtil.processTopLevelClasses(
          ...
          psiFile.getProject(),
          JSResolveUtil.getSearchScope(psiFile),
          onlyFqns,
          false
        );
      }
  ...
  private static PsiElement handleContentChange(PsiElement elt, TextRange range, String newElementName) {
    if (elt instanceof XmlTag || elt instanceof XmlAttributeValue) {
      ...
      if (i != -1) newElementName = newElementName.substring(0, i);
      return ElementManipulators.getManipulator(elt).handleContentChange(
        elt,
        range,
        newElementName
      );
