...
public class JSChangeUtil {
  @NonNls private static final String DUMMY = "dummy.";
  ...
  public static ASTNode createNameIdentifier(Project project, String name, IElementType type) {
    ...
    } else if (type == JSTokenTypes.STRING_LITERAL &&
               !StringUtil.isQuotedString(name)
              ) {
  ...
  private static @Nullable PsiElement createJSTreeFromTextImpl(Project project, @NonNls String text, @Nullable JSLanguageDialect dialect) {
    ParserDefinition def = dialect != null ? LanguageParserDefinitions.INSTANCE.forLanguage(dialect)
  ...
  public static PsiElement doAddAfter(final PsiElement jsElement, final PsiElement element, final PsiElement anchor) throws
                                                                                                                     IncorrectOperationException {
    if (!JSChangeUtil.isStatementOrComment(element) && !(element instanceof PsiWhiteSpace)) {
      throw new UnsupportedOperationException("js statement or whitespace expected");
    }
  ...
  public static String getSemicolon(final Project project) {
    return CodeStyleSettingsManager.getInstance(project).getCurrentSettings().getCustomSettings(JSCodeStyleSettings.class).USE_SEMICOLON_AFTER_STATEMENT ? ";":"";
  }
