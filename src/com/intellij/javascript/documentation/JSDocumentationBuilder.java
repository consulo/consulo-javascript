...
class JSDocumentationBuilder implements JSDocumentationProcessor {
  ...
  private @NonNls StringBuilder result;
  ...
  @NonNls private static final String BR_DELIMITER = "<BR>\n";
  ...
  public boolean onCommentLine(@NotNull String line) {
    ...
    if (trimmedLine.length() == 0) {
      ...
      if (myNewLinesPendingCount < maxSubsequentBr) ++myNewLinesPendingCount;
    ...
    if (!seenPre && line.indexOf('<') != -1) {
      ...
      while(matcher.find()) {
        ...
        if (tagNameThatDoNotNeedEscaping(s)) continue;
  ...
  public boolean onPatternMatch(@NotNull MetaDocType metaDocType,
                             @Nullable String matchName,
                             @Nullable String matchValue,
                             @Nullable String remainingLineContent,
                             @NotNull final String line, final String patternMatched) {
    ...
    if (metaDocType == MetaDocType.OPTIONAL_PARAMETERS) {
      ...
      if (currentParameterInfo != null) currentParameterInfo.optional = true;
    ...
    if (metaDocType == MetaDocType.PRIVATE ||
      ...
      if (generationInfo.visibility == null) generationInfo.visibility = s;
      else generationInfo.visibility += ", " + s;
      return true;
    }
    ...
    if (function != null) {
      ...
      if (metaDocType == MetaDocType.CONSTRUCTOR) {
        ...
        if (info != null) {
          if (matchValue != null) info.type = matchValue;
        ...
        if (matchName != null) methodGenerationInfo.returnInfo.type = matchName;
        if (matchValue != null) methodGenerationInfo.returnInfo.description.append(matchValue);
        if (remainingLineContent != null) methodGenerationInfo.returnInfo.description.append(remainingLineContent); 
      }
    }
    return true;
  }
  ...
  private void startFunction(JSFunction function) {
    ...
    if (parent instanceof JSAssignmentExpression) {
      ...
      if (functionName == null) functionName = expression.getText();
    ...
    for (Map.Entry<String, ParameterInfo> parameterInfo : ((MethodInfo)generationInfo).parameterInfoMap.entrySet()) {
      ...
      if (parameterType != null) {
        ...
        if (optional) result.append(", optional");
        result.append(" ] ");
      }
