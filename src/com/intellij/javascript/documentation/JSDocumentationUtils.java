...
public class JSDocumentationUtils {
  private static final @NonNls Pattern ourDojoParametersPattern = Pattern.compile("^\\s*(\\w+):(.*)$");
  private static final @NonNls Pattern ourJSDocParametersPattern = Pattern.compile(
    "^\\s*@param\\s*(?:\\{([^\\}]+)\\}\\s*)?(\\w+)?(?:\\s:\\s(\\S+))?(?:\\s*\\[(\\w+)(?:\\.(\\w+))?(?:=([^\\]]*))?\\])?(.*)$"
  );
  private static final @NonNls Pattern ourJSDocEventPattern = Pattern.compile("^\\s*@event\\s*(\\w+)(?:\\s:\\s(\\S+))?(.*)$");
  private static final @NonNls Pattern ourJSDocRemarkPattern = Pattern.compile("^\\s*@remarks (.*)$");
  private static final @NonNls Pattern ourJSDocMethodPattern = Pattern.compile("^\\s*@method\\s*(\\w+)(.*)$");
  private static final @NonNls Pattern ourJSDocClassPattern = Pattern.compile("^\\s*@class\\s*(\\w+(?:\\.\\w+)*)(.*)$");
  private static final @NonNls Pattern ourJSDocDeprecatedPattern = Pattern.compile("^\\s*@deprecated\\s*(.*)$");

  private static final @NonNls Pattern ourJSDocConstructorPattern = Pattern.compile("^\\s*@constructor$");
  private static final @NonNls Pattern ourJSDocFinalPattern = Pattern.compile("^\\s*@final$");
  private static final @NonNls Pattern ourJSDocPrivatePattern = Pattern.compile("^\\s*@private$");
  private static final @NonNls Pattern ourJSDocPublicPattern = Pattern.compile("^\\s*@public$");
  private static final @NonNls Pattern ourJSDocProtectedPattern = Pattern.compile("^\\s*@protected$");

  private static final @NonNls Pattern ourJSDocOptionalPattern = Pattern.compile("^\\s*@optional(.*)$");
  private static final @NonNls Pattern ourJSDocStaticPattern = Pattern.compile("^\\s*@static$");
  private static final @NonNls Pattern ourJSDocSeePattern = Pattern.compile("^\\s*@see (.*)$");
  private static final @NonNls Pattern ourJSDocDescriptionPattern = Pattern.compile("^\\s*@description\\s*(.+)$");
  private static final @NonNls Pattern ourJSDocReturnPattern = Pattern.compile("^\\s*@return(?:s)?\\s*(?:(?:\\{|:)?\\s*([^\\s\\}]+)\\s*\\}?\\s*)?(.*)$");
  private static final @NonNls Pattern ourJSDocNamespacePattern = Pattern.compile("^\\s*@namespace\\s*([\\w\\.]+)(.*)$");

  private static final @NonNls Pattern ourJSDocPropertyPattern = Pattern.compile("^\\s*@property\\s*(\\w+)(.*)$");
  private static final @NonNls Pattern ourJSDocTypePattern = Pattern.compile("^\\s*@type\\s*\\{?([^\\s\\}]+)\\}?(.*)$");
  private static final @NonNls Pattern ourJSDocRequiresPattern = Pattern.compile("^\\s*@requires\\s*(\\S+)(.*)$");
  private static final @NonNls Pattern ourJSDocDefaultPattern = Pattern.compile("^\\s*@default\\s*(.*)$");
  private static final @NonNls Pattern ourJSDocExtendsPattern = Pattern.compile("^\\s*@extends\\s*(.*)$");

  private static final @NonNls Map<Pattern,String> patternToHintMap = new HashMap<Pattern, String>();
  private static final @NonNls Map<Pattern, JSDocumentationProcessor.MetaDocType> patternToMetaDocTypeMap = new HashMap<Pattern, JSDocumentationProcessor.MetaDocType>();
  static {
  ...
  private static final @NonNls Map<Pattern,String> prefixToPatternToHintMap = new HashMap<Pattern, String>();
  static {
    prefixToPatternToHintMap.put(
      Pattern.compile("^\\s*description:(.*)$"),
      "descr"
    );
  ...
  public static void processDocumentationTextFromComment(ASTNode _initialComment, JSDocumentationProcessor processor) {
    ...
    while(commentLineIterator.hasMoreElements()) {
        ...
        if (matcher.matches()) {
          ...
          if (docType != null) {
            ...
            if (reportAboutFieldInParameter) {
              if (!processor.onPatternMatch(JSDocumentationProcessor.MetaDocType.FIELD, matchName,null, matcher.group(groupForFieldName), commentText,
                                             matched)) break;
            } else {
              if (!processor.onPatternMatch(docType, matchName, matchValue, remainingLineContent, commentText, matched)) break;
            }
            if (reportAboutOptionalParameter) {
               if (!processor.onPatternMatch(JSDocumentationProcessor.MetaDocType.OPTIONAL_PARAMETERS, matchName,fieldName, null, commentText,
                                             matched)) break;
            }

            if (reportAboutDefaultValue) {
              if (!processor.onPatternMatch(JSDocumentationProcessor.MetaDocType.DEFAULT, matchName,fieldName, matcher.group(groupForInitialValue), commentText,
                                             matched)) break;
            }
          }
      ...
      if (!matchedSomething && needPlainCharData) {
        if (!processor.onCommentLine(commentText)) break;
      }
    }
  }
  ...
  public static String unwrapCommentDelimiters(String text) {
    ...
    else if (text.startsWith("/*") || text.startsWith("//")) text = text.substring(2);

    if (text.endsWith("*/")) text = text.substring(0,text.length()-2);
    return text;
  }

  static @Nullable ASTNode findTrailingCommentInFunctionBody(final @NotNull JSFunction function) {
  ...
  static @Nullable ASTNode findLeadingCommentInFunctionBody(final @NotNull PsiElement element) {
    ...
    for(ASTNode node = block.getFirstChildNode().getTreeNext(); node != null; node = node.getTreeNext()) {
      ...
      if (nodeType != TokenType.WHITE_SPACE
        ...
        if (JSTokenTypes.COMMENTS.contains(nodeType)) return node;
        return null;
      }
    }
  ...
  public static PsiElement findDocComment(PsiElement element, PsiElement context) {
    ...
    if (element instanceof JSAttributeListOwner && context == null) {
      ...
      if (anchor != null) element = anchor;
    }

    if (!element.isValid() || element.getContainingFile() == null) return docComment;
    ...
    for(PsiElement prev = element.getPrevSibling(); prev != null; prev = prev.getPrevSibling()) {
      ...
      if (prev instanceof PsiComment) {
        docComment = prev;
        if (((PsiComment)prev).getTokenType() == JSTokenTypes.DOC_COMMENT) break;
        ...
        if (!propName.equals(propNameFromPrev)) break;
  ...
  private static @Nullable String getPropertyNameFromExprStatement(@NotNull PsiElement element) {
  ...
  private static String findTypeFromParameter(final JSVariable parameter, final PsiElement docComment) {
    if (docComment != null) {
      ...
      processDocumentationTextFromComment(docComment.getNode(), new JSDocumentationProcessor() {
        ...
        public boolean onCommentLine(@NotNull final String line) { return true; }
            ...
            for(MetaDocType expectedType:expectedTypes) {
              if (type == expectedType) {
                if (type == MetaDocType.TYPE && _anchor instanceof JSFunction) {
                  ...
                  if (jsParameterList != null && jsParameterList.getParameters().length > 0) return true;
                  if (_anchor.getParent() instanceof JSProperty) return true;
  ...
  public static void appendHyperLinkToElement(@Nullable PsiElement element, String elementName, final StringBuilder buffer,
    ...
    String fileName = containingFile == null ? null:!JSResolveUtil.isPredefinedFile(containingFile)?
    ...
    DocumentationManager.createHyperlink(
      buffer,
      (fileName != null ? fileName+":":"")+ elementName + (element != null ? ":"+element.getTextOffset():""),
  ...
  private static PsiElement findFunctionComment(JSParameter parameter) {
    ...
    if (anchor != null) return findDocComment(anchor);
    return null;
  }
  ...
  public static boolean findOptionalStatusFromComments(final JSParameter parameter) {
    ...
    processDocumentationTextFromComment(docComment.getNode(), new JSDocumentationProcessor() {
      ...
      public boolean onPatternMatch(@NotNull final MetaDocType type,
