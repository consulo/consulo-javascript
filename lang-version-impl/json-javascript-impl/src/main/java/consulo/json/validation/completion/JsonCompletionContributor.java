...
public class JsonCompletionContributor extends CompletionContributor {
    public JsonCompletionContributor() {
        extend(
            ...
            StandardPatterns.psiElement(JSTokenTypes.IDENTIFIER),
        ...
        extend(
            CompletionType.BASIC,
            StandardPatterns.psiElement(JSTokenTypes.SINGLE_QUOTE_STRING_LITERAL),
            new CompletionProvider() {
                @RequiredReadAction
                @Override
                public void addCompletions(
        ...
        extend(
            CompletionType.BASIC,
            StandardPatterns.psiElement(JSTokenTypes.STRING_LITERAL),
            new CompletionProvider() {
                @RequiredReadAction
                @Override
                public void addCompletions(
    ...
    private static void addVariants(@Nonnull CompletionParameters parameters, @Nonnull CompletionResultSet result, boolean quotes) {
        ...
        if (parent instanceof JSObjectLiteralExpression objectLiteralExpression) {
            JSProperty[] parentProperties = objectLiteralExpression.getProperties();
            alreadyDefined = new HashSet<>(parentProperties.length);
